---
description: 목적별 추천 읽기 순서를 안내합니다.
---

# 추천 읽기 가이드

> 목적에 따라 아래 경로를 추천합니다. 각 단계는 이전 단계의 내용을 기반으로 합니다.

## 클라우드를 처음 접하는 분

1. [클라우드란?](what-is-cloud.md)
2. [벤더 비교하기](compare-clouds.md)
3. [리전과 가용영역](regions-and-zones.md)
4. [공동 책임 모델](shared-responsibility.md)
5. [비용 구조 이해하기](pricing-model.md)

## 멀티클라우드를 검토하는 분

1. [멀티클라우드 이해하기](why-multicloud.md)
2. [벤더 비교하기](compare-clouds.md)
3. [벤더 선택 의사결정](decision-framework.md)
4. [멀티클라우드 네트워킹](multicloud-networking.md) → [커넥티비티 심화](../networking/multicloud-connectivity.md)
5. [하이브리드/엣지 컴퓨팅](../compute/hybrid-and-edge.md)

## 실무 운영을 준비하는 분

1. [계정과 조직 구조](accounts-and-organizations.md) → [랜딩존](../governance/landing-zone.md)
2. [비용 구조 이해하기](pricing-model.md) → [FinOps](../governance/finops.md)
3. [Well-Architected Framework](well-architected.md)
4. [재해복구](../governance/dr.md)
5. [모니터링](../devops/monitoring.md) → [SLI/SLO](../devops/slo.md) → [통합 관찰가능성](../devops/observability.md)
6. [원격 접근 관리](../devops/remote-access.md)
7. [패치 관리와 취약점 대응](../devops/patch-and-vulnerability.md)

## 보안 강화를 담당하는 분

1. [공동 책임 모델](shared-responsibility.md)
2. [IAM 개요](iam-overview.md) → [IAM 심화](../security/iam.md)
3. [시크릿 관리](../security/secrets.md)
4. [데이터 보호](../security/data-protection.md)
5. [보안 태세 관리](../security/security-posture.md)
6. [제로 트러스트](../security/zero-trust.md)
7. [DevSecOps](../devops/devsecops.md)
8. [보안 사고 대응](../security/incident-response.md)

## 마이그레이션/모더나이제이션 계획 중인 분

1. [애플리케이션 마이그레이션](../compute/migration.md)
2. [데이터베이스 마이그레이션](../database/migration.md)
3. [스토리지 마이그레이션](../storage/migration.md)
4. [애플리케이션 모더나이제이션](../compute/modernization.md)
5. [벤더 종속성과 출구 전략](../governance/exit-strategy.md)

## AI 도입을 검토하는 분

1. [클라우드 AI 시작하기](../ai/getting-started.md)
2. [AI와 머신러닝 서비스](../ai/ai-ml.md) — 모델 선택, 플랫폼 비교
3. [벡터 스토어와 AI 데이터](../ai/vector-store.md)
4. [RAG 고급 패턴](../ai/rag-patterns.md)
5. [프롬프트 엔지니어링](../ai/prompt-engineering.md)
6. [멀티클라우드 AI 아키텍처](../ai/multicloud-ai.md) — 비용, GPU, 벤더 조합
7. [LLMOps](../ai/llmops.md) — 평가, 운영, 비용 추적
8. [AI 보안](../security/ai-security.md) — 가드레일, 프롬프트 인젝션 방어
