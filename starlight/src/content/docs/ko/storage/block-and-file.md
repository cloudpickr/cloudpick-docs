...
스냅샷을 통한 백업과 복구에 대해서는 [백업과 복구](../../storage/backup/)에서 다룹니다.
...
:::note
백업 주기 설계, 3-2-1 규칙, 랜섬웨어 대비, DR과의 관계 등 상세는 [백업과 복구](../../storage/backup/)를 참고하세요.
:::

## 자주 하는 실수

- **파일 스토리지를 웹 서버 배포 원본으로 사용** — 온프레미스 NAS 패턴을 그대로 가져와 객체 스토리지 대비 5~10배 비용 발생
- **볼륨 크기를 과도하게 크게 프로비저닝** — 축소가 불가능하므로 나중에 줄일 수 없음. 필요한 만큼만 할당하고 온라인 확장 활용
- **블록 스토리지의 AZ 종속성을 간과** — 다른 AZ의 인스턴스에 볼륨을 연결할 수 없어 AZ 장애 시 복구가 지연됨

## 체크리스트

- [ ] 워크로드 특성(DB, 공유 파일, HPC)에 맞는 스토리지 유형(블록/파일/객체)을 선택했는가
- [ ] 블록 볼륨의 AZ 종속성을 고려하여 스냅샷 기반 복구 또는 리전 복제를 구성했는가
- [ ] 파일 스토리지는 여러 서버의 동시 접근이 필요한 경우에만 사용하고, 정적 콘텐츠는 객체 스토리지 + CDN을 사용하는가
