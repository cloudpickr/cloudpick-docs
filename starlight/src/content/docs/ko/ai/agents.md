...
:::note
엔터프라이즈 Desktop Agent의 만족도는 **모델 성능보다 IT의 데이터소스 연결 범위**에 좌우됩니다. 이 세팅을 조직 단위로 체계적으로 하는 것이 AX입니다 — [에이전트 도입 가이드](agent-adoption.md) 참고.
:::
...
| **관측** | OpenTelemetry 기반 트레이싱. 에이전트 특화 지표는 [LLMOps](llmops.md) 참고 |
| **보안** | 프롬프트 인젝션, 권한 상승, 데이터 유출, 무한 루프. 상세는 [AI 보안](../security/ai-security.md) 참고 |
...
- [ ] 비용 예산 및 서킷 브레이커
- [ ] 도입 전략은 [에이전트 도입 가이드](agent-adoption.md) 참고

## 관련 문서

- [에이전트 도입 가이드](agent-adoption.md) — AX 전략, 롤아웃, 거버넌스
