...
생성형 AI와 전통 ML 모델이 현대 클라우드 애플리케이션의 핵심 구성 요소가 되면서, 기존 코드 배포 파이프라인(DevOps)뿐 아니라 데이터셋, 프롬프트 템플릿, 모델 평가를 자동화하는 **MLOps / LLMOps**와의 유기적 통합이 중요해졌습니다.

:::note
AI 시스템의 7단계 수명주기, 배포 단계별 책임 경계, 그리고 DevOps 파이프라인과의 구체적인 연계 방식은 [AI 시스템 수명주기와 엔지니어링](../../ai/lifecycle/)을 참고하세요.
:::
