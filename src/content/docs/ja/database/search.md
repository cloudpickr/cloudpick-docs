...
:::note[前提知識と関連ドキュメント]
トランザクションデータの保存は、[マネージドRDB](../../database/managed-rdb/)、[NoSQL](../../database/nosql/)を先に参照してください。本ドキュメントは、全文検索・ログ分析・形態素解析などの検索エンジンワークロードに焦点を当てます。AIベースのセマンティック検索は[ベクトルストア](../../ai/vector-store/)と連携します。
:::
