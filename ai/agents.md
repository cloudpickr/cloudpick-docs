...
코딩 에이전트마다 "사람이 어디까지 정의하고, 에이전트가 어디서부터 실행하는가"의 경계가 다릅니다.
...
**Spec-driven 개발**은 "무엇을 만들지"를 먼저 문서화하고, 에이전트가 해당 스펙을 실행하는 방식입니다. 장점은 의도와 구현의 추적성(traceability)이 높고, 팀 단위에서 리뷰 포인트가 명확하다는 점입니다. 반면 탐색적 프로토타이핑에는 오버헤드가 됩니다.

**운영 관점에서의 시사점:**

- **변경 추적** — Spec-driven은 스펙 변경 이력이 곧 의사결정 로그. 감사(audit)와 규정 준수에 유리
- **재현성** — 스펙이 있으면 에이전트를 교체해도 동일 결과를 기대할 수 있어 벤더 종속 완화
- **온보딩** — 새 팀원이 "왜 이렇게 만들었는지"를 스펙에서 파악 가능
- **CI/CD 연동** — Kiro의 Hooks처럼 스펙 변경 시 자동으로 테스트/배포 트리거 가능
- **비용 예측** — 스펙 범위가 명확하면 에이전트 토큰 소비를 사전에 추정 가능
