...
동일한 파운데이션 모델(FM)을 여러 경로로 소비할 수 있습니다. **1P**(First-Party, 모델 제공사 직접 API)와 **3P**(Third-Party, 클라우드 플랫폼 경유)는 가격은 비슷해도 **쿼터, 기능, 빌링, 거버넌스**가 다릅니다.

| 용어 | 의미 | 예시 |
| --- | --- | --- |
| **1P** | 모델 제공사의 직접 API/콘솔 | api.openai.com, api.anthropic.com, console.upstage.ai |
| **3P** | 클라우드 벤더 플랫폼에서 호스팅 | Azure Foundry, Amazon Bedrock, Vertex AI |
| **마켓플레이스** | 클라우드 마켓플레이스를 통한 구독/프라이빗 오퍼 | AWS Marketplace, Azure Marketplace |

{% hint style="info" %}
"같은 모델인데 왜 채널에 따라 경험이 다른가?"를 이해하는 것이 이 문서의 목적입니다. 모델 자체의 성능 비교는 [AI 플랫폼과 모델 비교](ai-ml.md)를 참고하세요.
{% endhint %}
...
## Seat 기반 vs API: 언제 무엇을 선택하는가

많은 기업이 혼란스러워하는 부분입니다. **Seat 기반**(ChatGPT Business/Enterprise, Claude Team/Enterprise)과 **API**(토큰 기반)는 용도가 다릅니다.
...
### API 이용 시: 1P 직접 vs 3P 플랫폼

엔터프라이즈가 API를 코드에서 호출할 때, **3P(Bedrock/Vertex/Azure Foundry)를 선택하는 경우가 많습니다.** 이유:

| 3P를 선택하는 이유 | 설명 |
| --- | --- |
| **기존 클라우드 커밋 소진** | AWS EDP, Azure EA/MACC가 있으면 LLM 비용을 기존 약정에서 차감 가능 — 별도 예산 확보 불필요 |
| **네트워크 격리** | VPC/Private Link로 트래픽이 퍼블릭 인터넷을 경유하지 않음. 1P는 Enterprise 전용이거나 별도 설정 필요 |
| **컴플라이언스 상속** | 클라우드 벤더의 기존 인증(HIPAA BAA, FedRAMP, SOC 2)을 그대로 상속 |
| **통합 빌링/거버넌스** | 기존 클라우드 청구서에 합산, 비용 할당 태그로 팀별 추적, IAM으로 접근 제어 |
| **단일 계약** | 조달 부서가 새 벤더를 추가할 필요 없이 기존 클라우드 계약 범위 내에서 사용 |
| **멀티모델 접근** | 하나의 플랫폼에서 OpenAI + Anthropic + 오픈소스 등 여러 모델을 동일 API로 전환 가능 |

| 그래도 1P 직접 API를 선택하는 이유 | 설명 |
| --- | --- |
| **최신 모델/기능 즉시** | 신규 모델과 베타 기능이 1P에 먼저 출시 (수일~수주 차이) |
| **풀 기능** | Fine-tuning 고급 옵션, Assistants, Realtime API 등 3P에 없거나 지연되는 기능 |
| **간단한 시작** | 카드 등록 즉시 사용. 클라우드 계정/VPC 설정 불필요 |
| **클라우드 비종속** | 특정 클라우드에 의존하지 않고 모델 제공사와 직접 관계 유지 |

{% hint style="info" %}
대규모 엔터프라이즈의 일반적 패턴: **프로덕션 워크로드는 3P** (거버넌스, 커밋 소진, 네트워크 격리) + **혁신/실험 팀은 1P** (최신 기능, 빠른 프로토타이핑). 양쪽을 병행하면서 모델 버전을 동기화하는 전략이 일반적입니다.
{% endhint %}
