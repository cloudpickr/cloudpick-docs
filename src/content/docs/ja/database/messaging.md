...
:::note[前提知識と関連ドキュメント]
メッセージングとイベントストリーミング(Kafka、SQS、EventBridge)は単なる通信手段ではなく、データベース間のトランザクション分離(Saga)、リアルタイムの変更データキャプチャ(CDC)、[データパイプライン](../../database/data-pipeline/)の収集層として、データアーキテクチャの中核要素です。本ドキュメントはその観点から非同期メッセージング・ストリーミングエンジンを比較します。イベント駆動アーキテクチャの設計は[モダナイゼーション](../../compute/modernization/)を参照してください。
:::
