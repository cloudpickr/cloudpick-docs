...
### 어떤 AI가 필요한지 판단하기 (페르소나 및 접근 방식)

AI 도입 방식은 해결하려는 비즈니스 문제뿐 아니라, **사용 주체(비기술직 vs 개발자 vs ML 엔지니어)**와 **UI/UX 접점 형태**에 따라 달라집니다.

| 단계 (도입 방식) | 대상 사용자 (페르소나) | UI/UX 접점 형태 | 대표 서비스 예시 | 적합한 유스케이스 |
| --- | --- | --- | --- | --- |
| **1. Buy (완성형 SaaS)** | 비기술직 일반 임직원 (노코드) | 업무 소프트웨어 내장형 코파일럿, 전사 웹 챗봇 | Microsoft 365 Copilot, Gemini for Workspace, Salesforce Agentforce | 이메일·보고서 초안 작성, 회의록 요약, CRM 고객 응대 |
| **2. Assemble (노코드 스튜디오)** | 현업 기획자, 시민 개발자 | 드래그앤드롭 비주얼 캔버스, 사내 메신저(Teams/Slack) 봇 | Microsoft Copilot Studio, Bedrock IDE (SageMaker Unified Studio), Vertex AI Agent Builder | 사내 취업규칙 Q&A 봇, 부서별 정형 데이터 수집 봇 |
| **3. Build (맞춤형 앱·API)** | 소프트웨어 엔지니어 (개발팀) | 자체 웹/모바일 UI, 백그라운드 헤드리스 에이전트, 백엔드 API | Amazon Bedrock API, Azure AI Foundry SDK, Gemini Enterprise API | 사내 시스템 연동 전용 에이전트, 고객 대면 앱 내 AI 기능 |
| **4. Train & Ops (풀스택 ML)** | ML 엔지니어, 데이터 과학자 | Jupyter/VS Code, MLOps 오케스트레이션 파이프라인 | SageMaker AI, Azure Machine Learning, Vertex AI Pipelines | 금융 사기 탐지(FDS), 대규모 시계열 예측, 특화 모델 파인튜닝 |

:::note
- **도입 방식의 본질**: 위 4단계는 순서대로 올라가는 성숙도 사다리가 아니라, **운영 부담(TCO)과 필요한 제어권(IP 보호·맞춤화) 간의 트레이드오프 축**입니다.
- **비기술직군·일반 업무 자동화**: 1단계(Buy) 또는 2단계(Assemble)에서 시작하는 것이 구축 비용과 출시 속도(Time to Market) 측면에서 가장 유리합니다.
- **사내 시스템 연동 및 고유 서비스 개발**: 3단계(Build)의 파운데이션 모델 API 및 에이전트 SDK를 활용합니다.
- **정형 데이터 예측 및 자체 모델 학습**: 4단계(Train & Ops)의 [ML 플랫폼](../../ai/ai-ml/#ml-플랫폼)을 참고하세요.
- **전체 AI 수명주기 및 엔지니어링**: 7단계 수명주기와 거버넌스, MLOps vs LLMOps 운영 체계는 [AI 시스템 수명주기와 엔지니어링](../../ai/lifecycle/)을 참고하세요.
:::
