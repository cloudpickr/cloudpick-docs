...
:::note[선행 지식 및 연결 문서]
EU 클라우드 규제 전반은 [EU 가이드](../../eu/), 데이터 보호 일반 원칙은 [데이터 보호](../../security/data-protection/)를 먼저 참고하세요. 이 문서는 GDPR 역외이전 체계와 소버린 클라우드 옵션에 초점을 둡니다.
:::
...
EU 집행위원회는 일본(2019), 한국(2021년 12월), 영국, 스위스, 캐나다, 미국(EU-US Data Privacy Framework, 2023) 등 주요 교역국에 대해 **적정성 결정**을 채택했습니다.

:::caution
**적정성 결정은 원칙적으로 EU → 대상국 이전 방향에 적용됩니다.** 즉 EU에서 적정성 인정 국가로 개인정보를 이전할 때는 SCC 등 별도 안전장치 없이 이전이 가능하지만, 반대 방향으로 비EU 기업이 EU 거주자의 개인정보를 직접 수집·처리하는 경우(GDPR 제3조 역외적용 대상)에는 GDPR 자체의 준수 의무가 별도로 적용됩니다. 적정성 결정이 GDPR 준수 의무 자체를 면제해주는 것은 아닙니다.
:::
