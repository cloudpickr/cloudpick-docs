...
- [ ] 모든 클라우드와 온프레미스의 CIDR이 겹치지 않도록 IP 주소 계획이 문서화되어 있는가?
- [ ] 연결 방식(VPN/전용 연결/Cloud Exchange)이 대역폭·비용·안정성 기준으로 선택되었는가?
- [ ] 양쪽 클라우드의 보안 그룹/방화벽 규칙이 클라우드 간 트래픽을 명시적으로 허용하는가?
