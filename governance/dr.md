...
세컨더리 리전을 고를 때는 지연 시간(복제 지연)과 함께 **데이터가 국경을 넘는지**를 확인해야 합니다. 같은 국가에 두 번째 리전이 있는 벤더는 데이터가 국외로 나가지 않는 DR 구성이 가능하고, 그렇지 않으면 가장 가까운 인접국 리전이 후보가 됩니다.

{% hint style="warning" %}
국외 리전을 DR 대상으로 사용할 경우, 해당 국가의 개인정보 보호 법제가 정한 데이터 국외 이전 요건을 충족해야 합니다. 데이터 주권이 엄격한 워크로드는 국내 DR이 가능한 벤더·리전 조합을 우선 검토하세요. 국가별 리전 현황과 국내 DR 가능 조합은 국가별 가이드를 참고하세요 — [한국](../korea/index.md) 등.
{% endhint %}
...
- **국외 DR 리전 사용 시 데이터 주권 미검토** — 개인정보 보호 법제의 국외 이전 요건을 확인하지 않아 규제 위반
