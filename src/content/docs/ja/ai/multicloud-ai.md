...
> 文書基準: 2026年9月 | 本文書は変化の速い領域であり、四半期ごとのレビュー対象です。
...
- **B200 / B300（Blackwell・Blackwell Ultra）** — 2026年にクラウドで実際に提供される主力世代。B300（Blackwell Ultra）はAWS EC2 P6-B300などでGAされリージョンが拡大中で、B200比でメモリ（288GB級）・量子化推論の効率が高い。ネイティブFP4などにより推論効率が高い傾向だが、実際のスループットはワークロードごとに測定
- **次世代（Vera Rubin、VR200など）** — 2026年下半期にパートナー供給が始まるが、割当により実際の可用性は限定的（2027年まで拡大見込み）。最新世代の導入時はリージョン・コミットメントの可用性を公式文書で確認
