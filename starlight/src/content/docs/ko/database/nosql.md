...
검색 엔진(Elasticsearch/OpenSearch 계열)은 NoSQL 인접 영역으로, 전문 검색과 로그 분석에 특화되어 있습니다. 벤더별 서비스 비교는 [검색 엔진](../../database/search/)을 참고하세요.
...
:::note
인메모리 캐시(Redis/Valkey)의 상세 비교는 [캐시와 인메모리](../../database/cache/)를 참고하세요.
:::
...
:::note
글로벌 분산 DB(DynamoDB Global Tables, Cosmos DB, Spanner 등)의 일관성 트레이드오프와 선택 기준은 [관리형 RDB — 글로벌 분산 DB](../../database/managed-rdb/#글로벌-분산-db)를 참고하세요.
:::
...
:::note
키 설계 안티패턴과 DB 운영 전반(커넥션 풀, 캐시 전략, HA)은 [데이터베이스 운영](../../database/operations/)을 참고하세요.
:::
