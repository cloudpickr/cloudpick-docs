...
{% hint style="warning" %}
**REST API에 CDN을 태울 것인가?** 응답이 사용자마다 다르거나(인증 기반, 개인화) 실시간성이 중요한 API는 캐싱하면 안 됩니다. 반면 공개 데이터(상품 목록, 환율, 공지사항)처럼 동일 요청에 동일 응답이 보장되는 API는 짧은 TTL(5~60초)로 캐싱하면 오리진 부하를 크게 줄일 수 있습니다. 캐싱 여부는 `Cache-Control` 헤더로 API별로 제어하세요.
{% endhint %}
