...
**한 명의 오케스트레이터**가 기준·분담·대기·합본·판정을 소유합니다. 워커는 도메인 리뷰만 하고, 최종 Must-Fix 목록은 코디네이터가 **원문을 다시 열어 재검증**한 뒤 확정합니다.

권장 3분담:

| 워커 | 범위 예 |
| --- | --- |
| A. Security + Governance | `security/*`, `governance/*` |
| B. AI + Compute | `ai/*`, `compute/*` (모델·가격·GA 표기는 변동이 빨라 전용 워커 권장) |
| C. 기반 + 교차 | `about-cloud/*`, `networking/*`, `storage/*`, `database/*`, `devops/*`, README, SUMMARY |

운영 원칙:

1. 프롬프트에 이 절의 기준, “리뷰 전용 / 파일 수정 금지”, 위 산출 골격을 넣습니다.
2. **병렬로 독립 태스크**를 먼저 모두 띄운 뒤, 완료를 모읍니다. (한 워커 끝날 때까지 다음을 미루지 않음)
3. 코디네이터는 워커 소견을 그대로 합치지 않고, **각 Must-Fix를 원문·공식 1차 소스로 재확인**합니다.
4. 최종 산출에 **오케스트레이션 로그 + 기계 검사 + 머지 판정 표 + (가능하면) 이전 리뷰 대비**를 넣습니다.
