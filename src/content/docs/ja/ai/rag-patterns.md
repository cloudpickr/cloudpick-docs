...
:::note
RAGの基礎(ベクトルストア・エンベディング・基本的な検索)は[AI入門](../../ai/getting-started/)のRAGセクションと[ベクトルストアとエンベディング](../../ai/vector-store/)を先にお読みください。本文書はその基礎の上で、**本番品質を高める高度なパターン**(チャンキング・リランキング・ハイブリッド検索・クエリ拡張・評価)を扱います。
:::
