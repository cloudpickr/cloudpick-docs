...
한국의 망분리 정책은 "모든 시스템을 동일 수준으로 분리"하는 일률적 접근에서, **데이터 등급에 따른 차등 보안**으로 정교화되고 있습니다. CSAP는 상·중·하 3등급제로 시스템 민감도에 따라 다른 격리 수준을 요구하며, N2SF(국가망보안체계) 논의도 기밀/민감/공개 등급별 차등 통제를 지향합니다. 이는 "분리 여부"의 이분법에서 "어떤 수준의 통제가 필요한가"로 정책 질문 자체가 바뀌고 있음을 의미합니다.
