...
サーバー(ノード)を直接管理せずにコンテナを実行するサービスです。AWS Fargate、Azure Container Apps、Google Cloud Cloud Run、OCI Container Instancesなどがあり、各製品の詳細比較は[サーバーレス](../../compute/serverless/#서버리스-컨테이너)文書を参照してください。
