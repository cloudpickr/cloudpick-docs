...
클라우드 간 데이터 이동에는 이그레스(outbound) 요금이 발생합니다. 대량의 데이터를 클라우드 간 대용량 데이터를 빈번하게 전송하는 아키텍처는 비용이 급격히 증가할 수 있습니다. 벤더별 무료 범위와 단가는 [비용 구조 이해하기](pricing-model.md)를 참고하세요.
...
- [ ] 멀티클라우드를 도입해야 하는 구체적인 비즈니스 또는 규제 이유를 문서화했는가?
- [ ] 클라우드 간 이그레스 비용과 지연시간이 워크로드 요건에 미치는 영향을 분석했는가?
- [ ] 팀 규모와 역량이 2개 이상의 벤더를 동시에 운영할 수 있는 수준인가?
