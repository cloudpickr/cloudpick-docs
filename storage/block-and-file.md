...
| 항목 | 온프레미스 | 클라우드 |
| --- | --- | --- |
| **블록 (SAN/DAS)** | FC/iSCSI로 LUN 연결, RAID 직접 구성 | API로 생성/연결, 벤더가 복제/내구성 보장 |
| **파일 (NAS)** | NetApp/Isilon 장비 구매, 용량 사전 계획 | 관리형 NFS/SMB, 서버리스 자동 확장 옵션 |
| **공통** | 장비 노후화 시 교체 필요, 용량 한계 | 온라인 확장, 스냅샷/복제 내장 |

{% hint style="info" %}
클라우드 블록 스토리지는 서버의 로컬 디스크처럼 보이지만 실제로는 네트워크 연결입니다. 온프레미스 DAS보다 지연이 약간 높을 수 있으나, 스냅샷·복제·온라인 확장이 기본 제공됩니다.
{% endhint %}
