...
:::note
**지금 시작한다면:** 대부분의 엔터프라이즈 AI 도입은 **생성형 AI**(FM API 호출)에서 시작합니다. 전통 ML은 이미 데이터 파이프라인이 있는 조직에서, 딥러닝은 이미지/음성 등 특화 워크로드에서 여전히 활발합니다. 이 문서는 생성형 AI 시작에 초점을 두되, 전통 ML/딥러닝이 필요한 경우도 안내합니다. 세대별 벤더 서비스·모델의 상세 비교는 [AI 플랫폼과 모델 비교](../../ai/ai-ml/)를 참고하세요.
:::

### 어떤 AI가 필요한지 판단하기 (페르소나 및 접근 방식)

AI 도입 방식은 해결하려는 비즈니스 문제뿐 아니라, **사용 주체(비기술직 vs 개발자 vs ML 엔지니어)**에 따라 달라집니다. 아래는 빠르게 자신의 위치를 파악하기 위한 요약이며, 제어권·책임 경계·대표 벤더 솔루션까지 담은 상세 매트릭스는 [AI 시스템 수명주기와 엔지니어링](../../ai/lifecycle/#4단계-엔터프라이즈-도입-매트릭스)을 참고하세요.

| 도입 방식 | 대상 사용자 (페르소나) | 한 줄 설명 |
| --- | --- | --- |
| **1. Buy (완성형 SaaS)** | 비기술직 일반 임직원 (노코드) | 완성된 코파일럿·챗봇을 구독해 즉시 사용 |
| **2. Assemble (노코드 스튜디오)** | 현업 기획자, 시민 개발자 | 노코드 스튜디오로 지식DB·워크플로를 조립 |
| **3. Build (맞춤형 앱·API)** | 소프트웨어 엔지니어 (개발팀) | FM API·SDK로 자체 앱과 에이전트를 개발 |
| **4. Train & Ops (풀스택 ML)** | ML 엔지니어, 데이터 과학자 | 모델 가중치·인프라를 직접 소유하고 학습·운영 |
...
이 아래부터는 가장 많이 선택되는 **생성형 AI(FM API)** 도입 순서를 다룹니다. 전통 ML 파이프라인(학습→배포→모니터링)은 [AI 플랫폼과 모델 비교 — ML 플랫폼](../../ai/ai-ml/#ml-플랫폼)을 참고하세요.
