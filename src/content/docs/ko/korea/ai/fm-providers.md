...
:::note
모델 버전과 라이선스 조건은 빠르게 바뀝니다. 아래 표는 2026년 9월 기준이며, 도입 전 각 사의 공식 문서에서 최신 조건을 재확인하세요.
:::

## 제공사별 현황

| 제공사 | 대표 모델 | 최신 버전(2026.9 기준) | 라이선스 | 주요 제공 채널 |
| --- | --- | --- | --- | --- |
...
| 업스테이지 | Solar | Solar Pro 4(에이전트 워크로드 특화, 512K 컨텍스트, 2026.8 발표)가 현행 플래그십. 이전 Solar Pro 3(2026.1), Solar Pro 2(2025.7). 경량 Solar Mini 4(2026.9), 오픈웨이트 Solar Open 2 병행 | Pro 계열은 폐쇄형 API, Open 계열은 오픈웨이트 | Upstage Console API, OpenRouter, Amazon Bedrock Marketplace, AWS SageMaker JumpStart, AWS Marketplace |
...
이들 중 업스테이지, SK텔레콤, LG AI연구원은 정부의 「독자 AI 파운데이션 모델」 프로젝트 2차 단계평가(2026년 8월 18일 결과 발표)를 통과해 3차 평가에 진출했습니다. 모티프테크놀로지스는 글로벌 AAII 평가 1위(47점)에도 불구하고 사용자 평가에서 저조해 탈락했습니다. 네이버클라우드와 NC AI는 1차 단계평가(2026년 1월)에서 이미 탈락한 바 있습니다. NC AI는 이후 범용 LLM보다 3D·음성·번역 등 버티컬 생성 AI로 포지셔닝을 이동하는 모습을 보이고 있습니다. 자세한 선정 경과는 [소버린 AI·독자 파운데이션 모델 정책](../../../korea/ai/sovereign-fm-policy/) 문서를 참고하세요.
...
글로벌 1P/3P 패턴 자체는 [1P vs 3P 모델 제공](../../../ai/1p-vs-3p/)을 참고하세요.

## 글로벌 모델 대비 선택 기준

국내 FM과 GPT·Gemini·Claude 등 글로벌 모델 중 무엇을 선택할지는 벤더 우열이 아니라 **워크로드 요건**에 따라 판단해야 합니다.

- **규제·주권 요건**: 망분리 규제([망분리와 네트워크 격리](../../../korea/security/network-isolation/) 참고)나 공공 조달([CSAP](../../../korea/security/csap/) 참고) 대상 워크로드는 국내 리전에서 서비스되는 모델이 유리한 경우가 많습니다. 데이터가 해외로 이전되면 안 되는 워크로드는 국내 제공사의 국내 데이터센터 기반 API를 우선 검토하세요.
...
- **비용**: 업스테이지 Solar Pro 4는 100만 토큰당 입력 0.30달러 / 출력 1.20달러 수준으로 공개되는 등 국내 모델이 가격 경쟁력을 내세우는 경우가 있습니다. 다만 초당 처리량, 컨텍스트 길이 등 조건이 다르므로 단순 토큰 단가만으로 비교하지 마세요.
...
- [소버린 AI·독자 파운데이션 모델 정책](../../../korea/ai/sovereign-fm-policy/)
- [CSAP (클라우드 보안 인증)](../../../korea/security/csap/)
- [망분리와 네트워크 격리 (한국)](../../../korea/security/network-isolation/)
