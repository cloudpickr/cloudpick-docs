...
- **Slurm** — HPC(고성능 컴퓨팅)에서 오래 쓰인 배치 스케줄러입니다. 대규모 사전학습이나, 기존 온프렘 HPC·Slurm 잡을 그대로 옮겨오는 경우에 마찰이 적습니다. 제출 스크립트와 레시피를 재사용할 수 있습니다.
- **쿠버네티스** — 컨테이너 표준 위에서 학습·추론·서빙을 한 클러스터에 섞어 돌리거나, 네임스페이스 격리·멀티테넌시가 필요할 때 유리합니다. 기존 쿠버네티스 에코시스템(오퍼레이터, 오토스케일러 등)을 그대로 활용합니다.
