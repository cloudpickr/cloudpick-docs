...
- 미국 연방/방산 격리 맥락: [미국 소개](../../us/) · [FedRAMP](../../us/fedramp/) · [ITAR/EAR](../../us/itar/)
- EU 금융 ICT·주권: [EU 소개](../../eu/) · [DORA](../../eu/dora/)
:::

**망분리**(Network Segregation)는 업무망과 인터넷망을 분리하여 외부 위협이 내부 시스템에 도달하지 못하도록 하는 통제 방식입니다. 인터넷을 통한 악성코드 유입, 원격 침투, 데이터 유출을 네트워크 경계에서 원천 차단한다는 점에서 오랫동안 합리적인 선택이었으며, 전 세계 규제 시장(금융, 공공, 의료, 방산)에서 핵심 보안 요건으로 적용됩니다.
...
| **규제 적용** | 최고 민감도 등급 시스템 (예: 한국 CSAP 상, 미국 DoD IL5+) | 대부분의 클라우드 워크로드 |
...
인터넷과 완전히 분리된 클라우드 환경이 필요한 경우, 각 벤더는 다음 옵션을 제공합니다. 단, 각국 규제 시장에서의 인증 상태(예: 한국 CSAP, 미국 FedRAMP, 일본 ISMAP)는 별도로 확인해야 합니다.
...
전 세계 규제 시장의 망분리 정책은 "모든 시스템을 동일 수준으로 분리"하는 일률적 접근에서, **데이터 등급에 따른 차등 보안**으로 진화하고 있습니다. "분리 여부"의 이분법에서 "어떤 수준의 통제가 필요한가"로 정책 질문 자체가 바뀌고 있습니다.

| 규제/기준 | 요구사항 | 클라우드 대응 |
| --- | --- | --- |
| **PCI DSS** (카드 결제, 글로벌) | CDE(카드 데이터 환경) 네트워크 격리 | 전용 VPC + 방화벽 + 로깅 |
| **국가별 공공/금융 규제** | 물리적·논리적 분리 요건과 등급 체계는 국가마다 다름 | 해당 국가 가이드 참조 |

국가별 규제 상세는 다음 문서에서 다룹니다:

- **한국** — 전자금융감독규정, CSAP 상/중/하 등급, ISMS-P, N2SF(국가망보안체계) C/S/O 분류: [망분리와 네트워크 격리 (한국)](../../korea/security/network-isolation/)
- **미국** — FedRAMP, DoD SRG Impact Level, CMMC 2.0: [FedRAMP](../../us/fedramp/) · [ITAR/EAR](../../us/itar/)
- **EU** — NIS2, DORA (금융 ICT 복원력): [DORA](../../eu/dora/) · [NIS2 + EU AI Act](../../eu/nis2-ai-act/)
- **일본** — ISMAP, 가버먼트 클라우드: [ISMAP](../../japan/ismap/)
- **싱가포르** — MTCS, MAS TRM: [MTCS](../../singapore/mtcs/)

:::note
대부분의 기관은 모든 시스템이 같은 등급이 아닙니다. 시스템별로 등급을 분류하고, 등급에 맞는 격리 수준을 차등 적용하는 것이 핵심입니다. "전부 최고 등급"이나 "전부 퍼블릭"이 아니라, 혼합 구성이 현실적입니다.
:::
...
- 국가별 규제 링크는 해당 국가 가이드를 참고하세요: [한국](../../korea/security/network-isolation/) · [미국](../../us/fedramp/) · [EU](../../eu/dora/)
