...
| 생태계/통합 | 기존 도구, 조직 역량, 벤더 친화도 | 이미 쓰는 플랫폼과 잘 붙는가? |
...
```markdown
NIST SP 800-145는 클라우드 컴퓨팅을 *"최소한의 관리 노력이나..."*로 정의합니다.
```
...
- **열 순서:** AWS → Azure → GCP → OCI (시장 점유율 순)를 준수합니다.
...
검증 스크립트는 현재 내부 링크와 `SUMMARY.md` 포함 여부를 확인합니다. 아래 항목은 작성자가 직접 확인해야 합니다.
