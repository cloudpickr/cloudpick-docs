...
- 米国連邦/防衛産業における隔離の文脈: [米国 紹介](../../us/) · [FedRAMP](../../us/fedramp/) · [ITAR/EAR](../../us/itar/)
- EU金融ICT・主権: [EU 紹介](../../eu/) · [DORA](../../eu/dora/)
:::

**網分離**(Network Segregation)は、業務網とインターネット網を分離することで、外部の脅威が内部システムに到達しないようにする統制方式です。インターネット経由のマルウェア流入、リモート侵入、データ漏洩をネットワーク境界で根本的に遮断するという点で、長らく合理的な選択とされてきました。世界各国の規制市場（金融、公共、医療、防衛産業）において中核的なセキュリティ要件として適用されています。
...
| **規制適用** | 最高機密度のシステム (例: 韓国CSAP上等級、米国DoD IL5+) | 大半のクラウドワークロード |
...
インターネットと完全に分離されたクラウド環境が必要な場合、各ベンダーは以下のオプションを提供しています。ただし、各国の規制市場における認証状況（例：韓国CSAP、米国FedRAMP、日本ISMAP）は別途確認が必要です。
...
世界各国の規制市場における網分離政策は、「すべてのシステムを同一水準で分離」する画一的アプローチから、**データ等級に応じた差等セキュリティ**へと進化しています。「分離するかどうか」の二分法から「どの水準の統制が必要か」へと政策上の問いそのものが変化しています。

| 規制/基準 | 要求事項 | クラウド対応 |
| --- | --- | --- |
| **PCI DSS** (カード決済、グローバル) | CDE(カードデータ環境)のネットワーク隔離 | 専用VPC＋ファイアウォール＋ロギング |
| **各国の公共/金融規制** | 物理的・論理的分離の要件と等級体系は国ごとに異なる | 該当する国ガイドを参照 |

国別の規制詳細は以下のドキュメントで扱っています:

- **韓国** — 電子金融監督規程、CSAP上/中/下等級、ISMS-P、N2SF（国家網セキュリティ体系）C/S/O分類: [網分離とネットワーク隔離（韓国）](../../korea/security/network-isolation/)
- **米国** — FedRAMP、DoD SRG Impact Level、CMMC 2.0: [FedRAMP](../../us/fedramp/) · [ITAR/EAR](../../us/itar/)
- **EU** — NIS2、DORA（金融ICTレジリエンス）: [DORA](../../eu/dora/) · [NIS2 + EU AI Act](../../eu/nis2-ai-act/)
- **日本** — ISMAP、ガバメントクラウド: [ISMAP](../../japan/ismap/)
- **シンガポール** — MTCS、MAS TRM: [MTCS](../../singapore/mtcs/)

:::note
大半の組織では、すべてのシステムが同じ等級ではありません。システムごとに等級を分類し、等級に見合った隔離水準を差等適用することが重要です。「すべて最高等級」や「すべてパブリック」ではなく、混合構成が現実的です。
:::
...
- 各国の規制リンクは該当する国ガイドを参照してください: [韓国](../../korea/security/network-isolation/) · [米国](../../us/fedramp/) · [EU](../../eu/dora/)
...
- [AWS Outposts ドキュメント](https://docs.aws.amazon.com/outposts/)

### Azure

- [Azure ネットワークセキュリティベストプラクティス](https://learn.microsoft.com/azure/security/fundamentals/network-best-practices)
- [Azure Private Link ドキュメント](https://learn.microsoft.com/azure/private-link/)
- [Azure Stack Hub ドキュメント](https://learn.microsoft.com/azure-stack/operator/)
