...
:::note
이 문서는 분산 학습의 병렬화 전략을 다룹니다. 클러스터 통신 계층·배치·패브릭은 [GPU 워크로드 특성과 레퍼런스 아키텍처](../workload-and-architecture/)를, 쿠버네티스 상의 스케줄링은 [GPU 쿠버네티스와 스케줄링](../kubernetes-and-scheduling/)을 참고하세요.
:::
...
:::note
여기 나오는 병렬화 방식과 프레임워크(DeepSpeed, Megatron-LM, PyTorch FSDP)는 CUDA/NCCL 위에서 돌아 벤더에 매이지 않습니다. 즉 클라우드를 바꿔도 전략 자체는 대체로 그대로 옮겨집니다. 다만 실제 속도는 [노드 간 통신망 성능](../workload-and-architecture/#노드-간-고속-통신-패브릭--벤더-매핑)에 좌우됩니다.
:::
...
- **스토리지 대역폭** — 수백 GB~수 TB급 체크포인트를 짧은 시간에 써야 하므로, [스토리지 계층](../workload-and-architecture/#레퍼런스-아키텍처--3계층-통신-모델)의 처리량이 병목이 됩니다.
- **비동기·분산 저장** — 학습을 멈추지 않고 백그라운드로 저장하거나, 각 GPU가 자신의 샤드만 병렬 저장해 시간을 단축합니다.
- **자동 재개** — 실패 감지 후 마지막 체크포인트에서 재개하는 흐름은 [추론 서빙·안정성·비용 최적화 — 안정성·장애 대응](../serving-reliability-cost/#안정성장애-대응)에서 다룹니다.

## 관련 문서

- **다음: 쿠버네티스·스케줄링·gang scheduling** — [GPU 쿠버네티스와 스케줄링](../kubernetes-and-scheduling/)
- **클러스터 통신 계층·패브릭·배치** — [GPU 워크로드 특성과 레퍼런스 아키텍처](../workload-and-architecture/)
- **장애 자동 재개·용량 운영** — [추론 서빙·안정성·비용 최적화](../serving-reliability-cost/)
