...
- **イグレスコストの認識** — ペタバイト級のデータはイグレスコストが数万〜数十万ドル（USD）に達する場合があります。なお、AWS・Azure・Google Cloudはいずれも他クラウド／オンプレミスへ完全に移行する場合のイグレスコストを免除しています（AWS・Azureは2024年3月、Google Cloudは2024年1月）。Google Cloudは2025年9月にEU/UKのマルチクラウド環境向けにData Transfer Essentialsを提供し、常時のイグレスコストも免除しています（EU Data Act対応）
