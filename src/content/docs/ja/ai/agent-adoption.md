...
エンタープライズのロールアウト過程で、内部の能力だけでは複雑なレガシー連携を解決しにくい場合は、モデルベンダーやソリューションベンダーのエンジニアが顧客環境に直接組み込まれ、初期の本番コードを共同所有する[現場デプロイ (Field Deployment)](../../about-cloud/field-deployment/)の協業モデルを活用できます。
...
- [現場デプロイ (Field Deployment)](../../about-cloud/field-deployment/) — 顧客環境への組み込み、本番コード所有、エージェンティックデリバリー
