...
**망분리**(Network Segregation)는 한국 공공·금융 부문이 보안 정책의 근간으로 오래 채택해온 통제 방식입니다. 업무망과 인터넷망을 분리하여 외부 위협이 내부 시스템에 도달하지 못하도록 합니다. 인터넷을 통한 악성코드 유입, 원격 침투, 데이터 유출을 네트워크 경계에서 원천 차단한다는 점에서 오랫동안 합리적인 선택이었습니다.

금융권(전자금융감독규정), 공공기관(국가정보보안기본지침), 의료(개인정보보호법) 등 규제 시장에서 핵심 보안 요건으로 적용됩니다. 다만 API 연동, SaaS 활용, 원격 근무 등 시스템 간 연결이 불가피해진 현대 환경에서는 "분리"의 의미와 구현 방식이 함께 진화하고 있습니다.
...
인터넷과 완전히 분리된 클라우드 환경이 필요한 경우, 각 벤더는 다음 옵션을 제공합니다. 단, 한국 공공 부문에서의 CSAP 인증 여부는 별도로 확인해야 합니다.
...
한국의 망분리 정책은 "모든 시스템을 동일 수준으로 분리"하는 일률적 접근에서, **데이터 등급에 따른 차등 보안**으로 정교화되고 있습니다. CSAP는 상·중·하 3등급제로 시스템 민감도에 따라 다른 격리 수준을 요구합니다.

**N2SF(국가망보안체계) 1.0**은 2025년 9월 NCSC(국가사이버안보센터)에서 정식 공개되었으며, 기존 망분리 중심 정책을 등급별 차등 보안으로 전환하는 프레임워크입니다.
...
| **N2SF C등급 (기밀)** | 공공 전용 인프라 또는 에어갭 환경 | 인터넷 완전 차단, 국내 인력 운영 |
| **N2SF S등급 (민감)** | VPC 격리 + 전용선 + 접근 통제 강화 | CSAP 중 등급과 유사한 수준 |
| **N2SF O등급 (공개)** | 퍼블릭 클라우드 기본 보안 | 최소 보안 요건 충족 |

{% hint style="info" %}
대부분의 기관은 모든 시스템이 같은 등급이 아닙니다. 시스템별로 등급을 분류하고, 등급에 맞는 격리 수준을 차등 적용하는 것이 N2SF의 핵심입니다. "전부 상 등급"이나 "전부 퍼블릭"이 아니라, 혼합 구성이 현실적입니다.
{% endhint %}
