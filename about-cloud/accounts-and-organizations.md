...
| 문제 | 설명 | 해결 방법 |
| --- | --- | --- |
| **보안 경계 부재** | 개발 환경의 보안 사고가 프로덕션에 영향 | 계정 분리 + 조직 정책 → [보안 경계와 IAM](#보안-경계와-iam의-관계) |
| **비용 추적 어려움** | 어떤 팀이 얼마 쓰는지 파악 불가 | 권한 경계와 비용 경계 분리 → [빌링 구조](#권한-경계와-비용-경계--같은가-다른가) |
| **권한 관리 복잡** | 과다 권한 부여, 세밀한 제어 어려움 | 계정 격리 + IAM 최소 권한 → [IAM 실무 설계](../security/iam.md) |
| **서비스 할당량 공유** | 한 팀이 한도 소진 시 다른 팀도 영향 | 계정 분리로 쿼타 독립 → [서비스 쿼타 관리](#서비스-쿼타-할당량-관리) |
