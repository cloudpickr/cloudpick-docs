...
:::note[前提知識と関連ドキュメント]
EUのクラウド規制全般は[EUガイド](../../eu/)、データ保護の一般原則は[データ保護](../../security/data-protection/)を先に参照してください。本ドキュメントは、GDPRの域外移転体系とソブリンクラウドのオプションに焦点を当てます。
:::

GDPR（General Data Protection Regulation、EU一般データ保護規則）は、2018年の施行以降も進化を続けています。近年の変化は、法文自体よりも**「個人データが物理的にどこで、誰によって処理されるか」**という要求が強化される方向で現れています。本文書は、グローバル企業がEUへクラウドワークロードを拡張したり、EU顧客データを処理したりする際に直面する域外移転体系と、これを背景に登場した「ソブリンクラウド（Sovereign Cloud）」オプションについて整理します。
...
| メカニズム | 説明 |
| --- | --- |
| **十分性認定（Adequacy Decision）** | EU欧州委員会が特定国の個人データ保護水준をGDPRと同等と認めた場合、その国への移転には追加の保護措置が不要 |
...
### 主な十分性認定の現況（日本、韓国、英国、米国DPFなど）

EU欧州委員会は日本（2019年）、韓国（2021年12月）、英国、スイス、カナダ、米国（EU-US Data Privacy Framework、2023年）など主要な貿易相手国に対して**十分性認定**を採択しています。

:::caution
**十分性認定は原則としてEU域内から当該国への移転（EU→対象国）方向に適用されます。** つまり、EUから十分性認定国へ個人データを移転する際にはSCCなどの別途の保護措置なしに移転が可能ですが、逆方向で非EU企業がEU居住者の個人データを直接収集・処理する場合（GDPR第3条の域外適用対象）には、GDPR自体の遵守義務が別途適用されます。十分性認定はGDPR遵守義務そのものを免除するものではありません。
:::

グローバル企業がEUリージョンのクラウドを利用してEU顧客データを処理したり海外へ転送したりする場合、移転方向にかかわらず、ベンダーとの契約（DPA、Data Processing Addendum）にSCCが含まれているか、そしてベンダーが提供するリージョン・ガバナンスオプションが自社のデータ分類要件を満たすかを確認する必要があります。
...
- **2025年2月26日**: 完成（第3段階） — サポートログ、ケースノートなど**プロフェッショナルサービスデータ（Professional Services Data）**までEU/EFTA域内保存に拡張、データレジデンシー・透明性強化の完了を発表（ただし、一部の運用上制限された域外アクセス・転送の例外は、文書化された条件の下で残っています）
...
:::caution
**EUCSの「主権性（Sovereignty）要件」は2026年9月現在も確定しておらず、議論が進行中です。**
...
- **ソブリンランディングゾーン設計との連携**: EUデータレジデンシー・処理管轄要件をランディングゾーンのガードレールに反映する具体的なパターンについては、[ランディングゾーン — ソブリンランディングゾーン](../../governance/landing-zone/#ソブリンランディングゾーン-sovereign-landing-zone)を参考にしてください。
