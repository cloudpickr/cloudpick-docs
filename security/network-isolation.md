...
- 한국 금융·공공 망분리·N2SF·CSAP: [망분리와 네트워크 격리 (한국)](../korea/security/network-isolation.md)
- 미국 연방/방산 격리 맥락: [미국 개요](../us/index.md) · [FedRAMP](../us/fedramp.md) · [ITAR/EAR](../us/itar.md)
- EU 금융 ICT·주권: [EU 개요](../eu/index.md) · [DORA](../eu/dora.md)
{% endhint %}

**망분리**(Network Segregation)는 업무망과 인터넷망을 분리하여 외부 위협이 내부 시스템에 도달하지 못하도록 하는 통제 방식입니다. 인터넷을 통한 악성코드 유입, 원격 침투, 데이터 유출을 네트워크 경계에서 원천 차단한다는 점에서, 여러 규제 시장이 오랫동안 보안 정책의 근간으로 채택해 왔습니다.

금융·공공·의료 등 규제 산업에서 핵심 보안 요건으로 적용됩니다(각국의 근거 규정과 등급 체계는 상단의 국가 가이드 참고). 다만 API 연동, SaaS 활용, 원격 근무 등 시스템 간 연결이 불가피해진 현대 환경에서는 "분리"의 의미와 구현 방식이 함께 진화하고 있습니다.
...
| **규제 적용** | 최상위 민감도 등급 (기밀·핵심 시스템) | 대부분의 클라우드 워크로드 (국가별 인증 등급 매핑은 국가 가이드 참고) |
...
| **에어갭** | 인터넷과 완전히 단절된 클라우드 환경 | 최고 수준 규제 |

### 벤더별 에어갭/전용 환경

인터넷과 완전히 분리된 클라우드 환경이 필요한 경우, 각 벤더는 다음 옵션을 제공합니다. 단, 이 옵션들이 각국 공공 인증 체계(예: 한국 CSAP, 미국 FedRAMP)에서 인정되는지는 별도로 확인해야 합니다.
...
세부 요건은 관할마다 다르지만 방향은 공통적입니다. 각국의 네트워크 격리 정책은 "모든 시스템을 동일 수준으로 분리"하는 일률적 접근에서, **데이터 등급에 따른 차등 보안**으로 정교화되고 있습니다. "분리 여부"의 이분법이 아니라 "어떤 수준의 통제가 필요한가"가 정책 질문이 되고 있습니다.

| 규제 유형 (예시) | 요구사항 | 클라우드 대응 |
| --- | --- | --- |
| **카드 결제** (PCI DSS — 글로벌) | CDE(카드 데이터 환경) 네트워크 격리 | 전용 VPC + 방화벽 + 로깅 |
| **금융 망분리 규제** (국가별) | 내부망/DMZ/외부망 구분, 전산실 분리 | VPC 분리 + Private Subnet + 전용선 |
| **공공 클라우드 인증** (국가별 등급제) | 등급에 따른 물리적/논리적 분리 | 등급별 격리 수준 차등 적용 |
| **정보보호 관리체계** (전 산업) | 네트워크 분리, 접근 통제, 암호화 | Security Group + NACL + VPC Endpoint + TLS |

{% hint style="info" %}
대부분의 조직은 모든 시스템이 같은 등급이 아닙니다. 시스템별로 등급을 분류하고, 등급에 맞는 격리 수준을 차등 적용하는 것이 핵심입니다. "전부 최고 등급"이나 "전부 퍼블릭"이 아니라, 혼합 구성이 현실적입니다. 국가별 등급 체계와 아키텍처 매핑(한국 CSAP·N²SF·전자금융감독규정, 미국 FedRAMP·ITAR, EU DORA 등)은 상단의 국가 가이드 문서를 참고하세요.
{% endhint %}
...
- 국가별 규제·인증 출처는 각 국가 가이드의 "참고하기"를 확인하세요 — [한국](../korea/security/network-isolation.md) · [미국](../us/index.md) · [EU](../eu/index.md)
