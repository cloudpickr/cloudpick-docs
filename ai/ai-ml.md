...
| --- | --- | --- | --- |
| **전통 ML** | 회귀, 분류, 클러스터링 | 정형 데이터 기반, 피처 엔지니어링 필요 | SageMaker, Azure ML, Vertex AI Training |
| **딥러닝** | CNN, RNN, Transformer | 비정형 데이터(이미지, 텍스트) 처리, GPU 필수 | GPU 인스턴스, 관리형 학습 플랫폼 |
| **강화학습** | 보상 기반 의사결정 | 게임, 로보틱스, 추천 시스템 | SageMaker RL, Vertex AI |
...
**패러다임 전환:** 전통 ML은 "데이터를 모아 모델을 직접 학습"하는 방식이었습니다. 생성형 AI 이후에는 "이미 학습된 파운데이션 모델을 API로 호출"하는 것이 주류가 되었고, 에이전틱 AI에서는 모델이 도구를 사용해 "스스로 작업을 완수"하는 단계로 진화하고 있습니다.
