...
:::note[선행 지식 및 연결 문서]
네트워크 격리·서브넷 설계는 [VPC와 서브넷](../../networking/vpc-subnet/), 신원·권한 제어는 [IAM](../../security/iam/)을 먼저 참고하세요. 이 문서는 퍼블릭 IP 노출 없이 인스턴스에 접근하는 관리형 원격 접근에 초점을 둡니다.
:::
...
:::note
**VPN 의존도 축소:** AWS Verified Access가 HTTP(S) 외에 TCP/SSH/RDP/DB 프로토콜까지 지원하면서, 지원 대상 프로토콜에 대해 VPN 의존도를 크게 줄일 수 있습니다. 각 벤더의 ZTNA 서비스 비교는 [제로 트러스트](../../security/zero-trust/)를 참고하세요.
:::
...
| 쉘에서 하던 일 | 대체 방법 |
| --- | --- |
| 로그 확인 (`tail -f`) | [모니터링](../../devops/monitoring/) 중앙화 (CloudWatch Logs / Azure Monitor / Cloud Logging) |
| 설정 파일 수정 | [구성 관리 서비스](../../security/secrets/) (Parameter Store, App Configuration) + 동적 리로드 |
| 패키지 설치/업데이트 | [Patch Manager](../../devops/patch-and-vulnerability/) + 골든 이미지 파이프라인 |
| 서비스 재시작 | Run Command 또는 Auto Scaling 인스턴스 교체 |
| 디스크 정리 | CloudWatch Agent 알림 + 자동 스크립트 (EventBridge → Lambda) |
| 디버깅 (strace, tcpdump) | [관찰가능성](../../devops/observability/) 도구로 대부분 해결. 불가피한 경우만 세션 접근 |
...
- 이 이벤트들은 [보안 태세 관리](../../security/security-posture/)의 GuardDuty/Security Hub와 연동하여 비정상 패턴을 자동 탐지할 수 있습니다
...
- **SSH 22번 포트를 0.0.0.0/0으로 개방** — 봇의 무차별 대입 공격에 즉시 노출됩니다. 관리형 접근 서비스를 사용하면 인바운드 포트 개방이 불필요합니다.
...
- [ ] 프로덕션 인스턴스에 퍼블릭 IP 없이 관리형 서비스(Session Manager/Bastion/IAP)로만 접근하는가?
