...
:::note[선행 지식 및 연결 문서]
파운데이션 모델과 플랫폼 기본은 [AI 시작하기](../../ai/getting-started/), 동일 FM을 어디서 소비할지(1P 직접 vs 3P 클라우드 경유)는 [LLM 채널 선택 가이드](../../ai/1p-vs-3p/)를 먼저 참고하세요. 이 문서는 FM 라이선스 등급과 비용 관리에 초점을 둡니다.
:::
...
| --- | --- | --- | --- |
| **Business** (구 Team) — Standard 석 | ~$25/석/월 (연납 $20) | 소규모 팀 (2석+) | — |
| **Business** — Premium 석 | ~$125/석/월 (연납 $100) | 고사용량 사용자 | Standard 대비 5배 사용량, 5시간 사용 제한 없음. 한 워크스페이스에서 석 유형 혼합 가능 |
...
| 플랜 | 가격 | 대상 | Team과의 차이 |
| --- | --- | --- | --- |
| **Team Standard** | ~$25/석/월 ([공식 가격](https://claude.com/pricing)) | 소규모 팀 (2–150석) | — |
...
:::note
위 표는 조직 차원의 **과금 모델·조달 거버넌스**(누가 얼마를 쓰는지, 어떻게 배분·청구하는지)에 초점을 둡니다. 프로덕션 런타임에서 요청 단위로 예산을 강제하고(Budget Guardrail) 모델 장애 시 전환하는(Fallback) 운영 통제는 [LLMOps](../../ai/llmops/)를, 모델별 추론 단가 절감(프롬프트 캐싱·배치·모델 라우팅)은 [AI 플랫폼과 모델 비교 — 추론 비용 최적화](../../ai/ai-ml/#추론-비용-최적화)를 참고하세요.
:::
...
- [LLM 채널 선택 가이드](../../ai/1p-vs-3p/) — 채널 패턴, Seat vs API 선택
- [AI 플랫폼과 모델 비교](../../ai/ai-ml/) — 모델 카탈로그, 추론 비용 최적화
- [LLMOps](../../ai/llmops/) — 런타임 비용 운영(Budget Guardrail, Fallback, 요청당 비용)
- [FinOps](../../governance/finops/) — 클라우드 비용 거버넌스 전반
- [에이전트 도입 가이드](../../ai/agent-adoption/) — 에이전트 비용 관리
