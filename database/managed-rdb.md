...
리전을 넘어 전 세계에 데이터를 분산하고, 각 리전에서 읽기/쓰기가 가능한 DB입니다. 일반 관리형의 "크로스 리전 읽기 복제본"과 달리, **멀티 리전 쓰기**를 지원합니다.

| 벤더 | 제품 | 특징 |
| --- | --- | --- |
| AWS | Aurora Global Database | 최대 5개 리전 읽기. 쓰기는 단일 리전 (1초 이내 복제) |
| GCP | Spanner | **멀티 리전 쓰기** + 강한 일관성. 글로벌 트랜잭션 지원 |
| Azure | Cosmos DB (관계형 모드) | 멀티 리전 쓰기. 5가지 일관성 수준 선택 가능 |
| OCI | Autonomous Data Guard | 크로스 리전 복제. 자동 페일오버 |

{% hint style="info" %}
글로벌 분산 DB는 비용이 높고 설계가 복잡합니다. "전 세계 사용자에게 저지연 쓰기가 필요한가"가 핵심 판단 기준입니다. 읽기만 분산하면 되는 경우 크로스 리전 읽기 복제본으로 충분합니다.
{% endhint %}
