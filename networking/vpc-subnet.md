...
| 온프레미스 대역 회피 | 온프레미스이 `172.16.0.0/12` 사용 중이면 클라우드는 `10.0.0.0/8` 사용 | 전용선 연결 시 충돌 방지 |
...
| **온프레미스 경로 전파** | Route Propagation (자동) | 자동 전파 (VPN Gateway) | Cloud Router (자동) | DRG Route Table |
...
| OCI | Dynamic Routing Gateway (DRG v2) | 허브 역할. VCN/온프레미스/타 리전 연결 |
...
| **온프레미스 연결** | VPC마다 VPN 필요 | 허브에 1개 VPN/전용선 |
| **라우팅 관리** | VPC마다 개별 관리 | 허브에서 중앙 관리 |
| **비용** | 데이터 전송만 | 시간당 + 데이터 처리 비용 |
| **적합한 경우** | VPC 2~3개, 단순 구조 | VPC 4개 이상, 온프레미스 연결, 중앙 관리 필요 |
