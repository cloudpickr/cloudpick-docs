...
온프레미스는 서버를 구매하는 순간 감가상각이 시작되는 **자본 지출** (CapEx)입니다. 사용하든 안 하든 비용이 발생합니다.

클라우드는 사용한 만큼만 내는 **운영 지출** (OpEx)입니다. 단, 아키텍처를 클라우드에 맞게 설계해야 이 이점을 누릴 수 있습니다. 온프레미스 방식 그대로 클라우드에 올리면(Lift & Shift) 오히려 비용이 증가할 수 있습니다.

| 항목 | 온프레미스 (CapEx) | 클라우드 (OpEx) |
| --- | --- | --- |
| 초기 투자 | 서버·네트워크 장비 구매 (수억 원) | 없음 |
| 월 비용 | 전력·냉각·인력 고정비 | 사용량 비례 변동비 |
| 유휴 리소스 | 비용 계속 발생 | 중지/삭제 시 비용 0 |
| 확장 시 | 추가 구매 (수 주) | 즉시 (분 단위) |
...
{% hint style="warning" %}
**클라우드 = 자동으로 저렴해지는 것은 아닙니다.** 클라우드의 이점을 누리려면 아키텍처를 클라우드 네이티브로 설계하고, 비용을 지속적으로 모니터링해야 합니다. 이에 대해서는 [비용 구조 이해하기](pricing-model.md)와 [FinOps](../governance/finops.md)를 참고하세요.
{% endhint %}
