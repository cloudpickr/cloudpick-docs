...
한국의 공공기관은 CSAP(클라우드 보안 인증) 획득 벤더만 사용할 수 있습니다. 글로벌 서비스는 AWS/GCP로, 공공 워크로드는 NCP/KT Cloud로 분리하는 패턴이 가장 흔합니다.
...
> CSAP 제도는 N2SF(국가망보안체계) 전환 등 변경이 진행 중이므로, 도입 시 최신 현황을 확인하세요.
...
클라우드 간 데이터 이동에는 이그레스(outbound) 요금이 발생합니다. 대량의 데이터를 클라우드 간에 주고받는 아키텍처는 비용이 급격히 증가할 수 있습니다.

| 벤더 | 이그레스 단가 (인터넷, 서울 리전 기준) |
| --- | --- |
