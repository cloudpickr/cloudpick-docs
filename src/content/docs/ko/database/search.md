...
- **RDB의 LIKE 검색으로 전문 검색을 대체하려 함** — 데이터가 늘어나면 풀 테이블 스캔으로 성능이 급격히 저하됩니다. 전문 검색이 필요하면 검색 엔진을 도입하세요.
- **샤드 수를 과도하게 설정** — OpenSearch/Elasticsearch에서 샤드가 너무 많으면 클러스터 오버헤드가 증가합니다. 샤드당 10~50GB를 기준으로 설계하세요.
- **로그 보존 기간을 무제한으로 설정** — 검색 엔진에 모든 로그를 영구 보관하면 스토리지 비용이 폭증합니다. 핫/웜/콜드 티어를 활용하고 오래된 로그는 객체 스토리지로 이관하세요.

## 체크리스트

- [ ] 검색 요구사항(키워드/시맨틱/하이브리드)을 정의하고 적합한 서비스를 선택했는가
- [ ] 인덱스 수명 주기 정책(ILM)과 로그 보존 기간을 설정했는가
- [ ] 한국어 검색이 필요한 경우 형태소 분석기(nori 등)를 구성했는가
