...
- **"Well-Architected는 신규 프로젝트에만 적용된다"** — 기존 워크로드를 점검하고 개선하는 데도 동일하게 활용할 수 있습니다. 정기 리뷰를 권장합니다.
- **"모든 Pillar를 동시에 완벽하게 충족해야 한다"** — Pillar 간에는 트레이드오프가 있습니다(예: 보안 강화 vs 비용 증가). 워크로드 우선순위에 맞게 균형을 잡으세요.
- **"리뷰 도구를 돌리면 자동으로 개선된다"** — 도구는 문제를 식별할 뿐, 실제 개선은 팀이 권장 사항을 검토하고 적용해야 합니다.

## 체크리스트

- [ ] 프로덕션 워크로드에 대해 Well-Architected 리뷰를 최소 1회 수행했는가?
- [ ] 조직에서 가장 중요한 Pillar(보안, 비용, 안정성 등)의 우선순위를 정의했는가?
- [ ] 리뷰에서 도출된 개선 사항을 백로그에 등록하고 추적하고 있는가?
