...
パターンを知ることより、**いつどの技法を使うか**を判断することが実務の核心です。状況・モデル特性・コストを合わせて考慮します。

| 状況 | 推奨技法 | 理由 / 注意 |
| --- | --- | --- |
| 単純な分類・抽出・変換 | Zero-shot | 例なしで十分。トークンの無駄を最小化 |
| 出力形式・トーンが難しい | Few-shot | 2〜5個の例で形式を「見せる」。例が偏らないように |
| 多段階推論・数学・論理 | Chain-of-Thought | 中間推論を誘導。**ただし推論特化モデル(o系、推論モード)はCoTを内蔵するため明示的CoTは不要または逆効果** |
| 構造化出力(JSONなど)が必須 | 出力形式の強制 + スキーマ | プロンプト指示より**ネイティブの構造化出力/関数呼び出しAPI**(対応時)の方が信頼性が高い |
| 外部ツール・検索と結合 | ReAct / ツール呼び出し | エージェントワークフロー。ループコスト・無限ループ防止策が必要 |
| 一貫した役割・ポリシー適用 | System Prompt / Persona | セッション全体に固定。ユーザー入力で上書きされないように |

**モデルタイプ別の考慮:**

- **汎用モデル(GPT-5.6、Claude Sonnetなど)**: Few-shot・CoTが品質を有意に向上させる。
- **推論特化モデル(o系、Claude推論モードなど)**: 内部で推論するため「段階的に考えて」等の指示は不要で、過度なCoT指示はむしろ性能・コストに不利になり得る。
- **コスト観点**: Few-shotの例やCoTは入力トークンを増やす。反復呼び出しが多い経路では例数・推論長を最小化するかプロンプトキャッシングを活用する。

:::note
どの技法が実際に優れているかは**評価で確認**する必要があります。代表的な入力セットに対して正答率・形式順守率・トークンコストをA/Bで測定し、下記の[改善の反復ループ](#プロンプト改善の反復ループ)へつなげます。
:::
