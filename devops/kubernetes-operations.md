...
- **클러스터 업그레이드를 미루다가 지원 종료 버전에 도달** — 강제 업그레이드나 보안 패치 중단으로 이어집니다. 분기마다 업그레이드 계획을 수립하세요.
- **Network Policy 없이 운영** — 기본값은 모든 Pod 간 통신 허용입니다. 침해 시 횡적 이동(lateral movement)이 무제한으로 가능합니다.
- **VPC 서브넷 IP를 작게 잡아 Pod 스케줄링 실패** — VPC CNI 환경에서 노드+Pod가 모두 VPC IP를 소비합니다. 초기 설계 시 충분한 CIDR(/20 이상)을 할당하세요.

## 체크리스트

- [ ] 클러스터 버전이 벤더 지원 범위 내에 있고, 업그레이드 일정이 수립되어 있는가?
- [ ] OPA/Kyverno 등으로 승인된 이미지 레지스트리만 허용하는 정책이 적용되어 있는가?
- [ ] Pod별 리소스 요청(requests)과 제한(limits)이 설정되어 있는가?
...
- [Kubernetes Networking Comparison (AKS vs EKS vs GKE vs ARO)](https://subnettool.com/kubernetes-networking-comparison) — 벤더별 네트워킹 차이 비교
