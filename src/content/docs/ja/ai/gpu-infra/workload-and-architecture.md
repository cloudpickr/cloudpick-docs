...
GPU 1枚で終わる仕事なら、この文書は必要ありません。GPU 1枚に収まらない大きなモデルを学習したり、サーバー1台では手に負えない規模になると、そこから**複数のGPUと複数のサーバーを一つにまとめて**仕事を分担させる必要があります。

このときよくある誤解が「もっと速いGPUを、もっとたくさん入れればその分だけ速くなる」というものです。実際はそうではありません。複数のGPUが協力するには、互いに計算結果を絶えずやり取りする必要があり、この**やり取りの速度（GPU間・サーバー間の通信）と、データをGPUまで運ぶ速度（ストレージ）**が足を引っ張ると、GPUをいくら増やしても遊んでいる時間だけが増えます。たとえるなら、料理人（GPU）を100人集めても、厨房が狭く食材を運ぶ通路が詰まっていれば100倍速くはならないのと同じです。
