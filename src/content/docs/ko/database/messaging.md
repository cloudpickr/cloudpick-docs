...
:::note[선행 지식 및 연결 문서]
메시징과 이벤트 스트리밍(Kafka, SQS, EventBridge)은 단순한 통신 수단이 아니라, 데이터베이스 간 트랜잭션 분리(Saga), 실시간 변경 데이터 캡처(CDC), [데이터 파이프라인](../../database/data-pipeline/)의 수집 계층으로서 데이터 아키텍처의 핵심 요소입니다. 이 문서는 그 관점에서 비동기 메시징·스트리밍 엔진을 비교합니다. 이벤트 기반 아키텍처 설계는 [모더나이제이션](../../compute/modernization/)을 참고하세요.
:::
...
- [서버리스](../../compute/serverless/) — 이벤트 트리거·비동기 호출과 큐 연동
- [API Gateway](../../networking/api-gateway/) — 동기 API 앞단과 비동기 백엔드 분리
- [데이터 파이프라인과 ETL](../../database/data-pipeline/) — 스트리밍·배치 파이프라인
- [통합 관찰가능성 아키텍처](../../devops/observability/) — 큐/스트림 지연·실패 관측
