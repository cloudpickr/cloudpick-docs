...
:::note
MTCSは、韓国のCSAPのように法律上強制される参入規制というよりも、**公共調達および金融業界のベンダーデューデリジェンスにおいて事実上の必須要件**として機能する業界標準に近いものです。IMDAが発注する政府クラウド調達（Government Commercial Cloudなど）では、MTCS認証が要求事項として明記されるのが一般的です。
:::
...
| **Level 3** | 規制産業、政府調達対象システム、機微・高影響データを処理するシステム | Level 1・2の統制に加え、厳格な追加統制を要求。**政府調達に参加しようとするCSPはLevel 3認証が事実上必須** |
...
- **韓国企業への示唆**: 韓国国内で既にCSAP・ISMS-P認証体系に精通しているアーキテクトであっても、シンガポールは独自の認証体系（MTCS）と独自の金融業界監査体系（OSPAR）を運用している点に留意する必要があります。両体系は相互承認されないため、シンガポール進出時には別途の認証・監査取得ロードマップを策定する必要があります。
