...
:::note[前提知識と関連ドキュメント]
トランザクションデータの保存は、[マネージドRDB](../../database/managed-rdb/)、[NoSQL](../../database/nosql/)を先に参照してください。本ドキュメントは、全文検索・ベクトル検索・ログ分析などの検索エンジンワークロードに焦点を当てます。生成AIの埋め込みパイプラインの観点でのベクトルストアは[ベクトルストア](../../ai/vector-store/)と連携します。
:::
