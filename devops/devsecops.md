...
{% hint style="info" %}
배포 후 운영 환경의 보안 감시 → [보안 태세 관리](../security/security-posture.md), OS/런타임 패치 → [패치 관리와 취약점 대응](../devops/patch-and-vulnerability.md)
{% endhint %}
...
소스 코드를 실행하지 않고 분석하여 보안 취약점을 찾습니다.
...
실행 중인 애플리케이션을 외부에서 공격하여 취약점을 찾습니다.
...
오픈소스 의존성의 알려진 취약점(CVE)과 라이선스 위반을 탐지합니다.
...
| AWS CodeGuru Security | 코드 리뷰 시 시크릿 노출 탐지 |
