...
:::tip[이 문서는 AI 도입 여정 지도입니다]
"왜 AI인가 → 무엇을 살까 → 어떻게 만들까 → 어떻게 운영·거버넌스할까 → 현장에 배포"의 전 주기를 이 문서가 조망합니다. [AI 시작하기](../../ai/getting-started/)에서 역할별 빠른 진입점을 고른 뒤, 이 문서의 [4단계 도입 매트릭스](#4단계-엔터프라이즈-도입-매트릭스)에서 자신의 역할·성숙도에 맞는 경로를 확인하세요.
:::
...
- **학습 파이프라인 구성** — 자체 모델을 훈련하거나 파인튜닝하는 경우 분산 학습 파이프라인을 구축합니다. 병렬화 전략(TP/PP/DP)과 GPU 클러스터 구성은 [GPU 인프라 — 분산 학습 표준 아키텍처](../../ai/gpu-infra/distributed-training/)를 참고하세요.
...
:::note[현장 배포(FDE)와의 연계]
설계된 AI 아키텍처를 고객사의 망분리·에어갭 인프라나 복잡한 레거시 환경에 실제 프로덕션 코드로 안착시키는 실행 모델의 하나로 [현장 배포 (Field Deployment)](../../about-cloud/field-deployment/)를 참고할 수 있습니다.
:::
...
자신의 티어·역할을 정했다면, 다음 단계의 실무 문서로 이동합니다.

- **경영·의사결정자 (Tier 1–2 판단)** — [AI 플랫폼과 모델 비교](../../ai/ai-ml/)로 후보를 좁히고, [LLM 채널 선택](../../ai/1p-vs-3p/)·[라이선스와 비용](../../ai/licensing/)으로 조달 방식을 정합니다.
- **아키텍트 (Tier 2–3 설계)** — 여기서 모델·제어권 경계를 정한 뒤 [멀티클라우드 AI](../../ai/multicloud-ai/)로 배치 아키텍처를, 보안 경계는 [AI 보안](../../security/ai-security/)으로 넘어갑니다.
- **개발자 (Tier 3 구축)** — [프롬프트 엔지니어링](../../ai/prompt-engineering/) → [RAG 고급 패턴](../../ai/rag-patterns/) → [AI 에이전트](../../ai/agents/) 순으로 구현합니다.
- **운영자·플랫폼 (Tier 3–4 운영)** — [LLMOps](../../ai/llmops/)로 평가·비용·관측 체계를 세웁니다.
- **거버넌스** — 이 문서의 [문제 정의·거버넌스](#1-문제-정의-및-거버넌스-problem-framing--governance) 단계와 [에이전트 도입 가이드](../../ai/agent-adoption/)의 롤아웃·거버넌스 프레임워크를 함께 봅니다.
- **현장 배포(FDE)** — 설계된 시스템을 고객의 망분리·레거시 환경에 프로덕션 코드로 안착시키는 관점은 [현장 배포](../../about-cloud/field-deployment/)를 참고합니다.
...
- **인프라 자동화 (IaC)** — 벡터 데이터베이스, GPU 노드 풀, 서빙 엔드포인트를 [IaC](../../devops/iac/)로 프로비저닝하여 환경 일관성을 보장합니다. GPU 노드 풀의 스케줄링·쿼터·공유는 [GPU 인프라 — GPU 쿠버네티스와 스케줄링](../../ai/gpu-infra/kubernetes-and-scheduling/)을 참고하세요.
