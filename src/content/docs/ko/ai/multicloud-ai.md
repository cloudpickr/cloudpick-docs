...
:::note
이 문서는 심화 내용입니다. AI 서비스 비교가 처음이라면 [AI 시작하기](../../ai/getting-started/)와 [AI 플랫폼과 모델 비교](../../ai/ai-ml/)를 먼저 읽는 것을 권장합니다.
:::
...
:::note
각 벤더의 AI 플랫폼 상세 비교, 모델 카탈로그, Fine-tuning 옵션은 [AI 플랫폼과 모델 비교](../../ai/ai-ml/)를 참고하세요.
:::
...
| **메모리 (H100 대비)** | 1× | ~1.8× | ~2.4× | ~4.8× (2×B200 합산) |

:::note
추론 처리량(토큰/초 등)은 모델·정밀도·배치·프레임워크·시스템 구성에 따라 크게 달라집니다. "H100 대비 n배" 같은 벤치마크 배수는 벤더 발표 자료의 특정 조건에 묶이므로, 도입 전 [NVIDIA 데이터시트](https://www.nvidia.com/en-us/data-center/)와 각 CSP 인스턴스 문서를 확인하세요.
:::

**선택 가이드:**

- **H100/H200** — 리전 가용성이 상대적으로 넓음. 중규모 학습, Fine-tuning, 일반 추론에 적합. H200은 H100과 동일 아키텍처 계열이며 메모리·대역폭이 커 긴 컨텍스트 추론에 유리
- **B200 / B300 (Blackwell·Blackwell Ultra)** — 2026년 클라우드에서 실제 제공되는 주력 세대. B300(Blackwell Ultra)은 AWS EC2 P6-B300 등으로 GA되어 리전이 확대되는 중이며, B200 대비 메모리(288GB급)·양자화 추론 효율이 더 높음. 네이티브 FP4 등으로 추론 효율이 높은 편이나 실제 처리량은 워크로드별로 측정
- **차세대(Vera Rubin, VR200 등)** — 2026년 하반기 파트너 공급이 시작되나 물량 배정으로 실제 가용은 제한적(2027년까지 확대 전망). 최신 세대 도입 시 리전·약정 가용성을 공식 문서로 확인
...
:::note
RAG 파이프라인 구현 패턴(하이브리드 검색, 청킹 전략, 리랭킹, 오케스트레이션)의 상세는 [RAG 고급 패턴](../../ai/rag-patterns/)을 참고하세요.
:::
