...
:::note[선행 지식 및 연결 문서]
DB 종류 선택은 [관리형 RDB](../../database/managed-rdb/), [NoSQL](../../database/nosql/)을 먼저 참고하세요. 서버·애플리케이션 이전 전반은 [컴퓨트 마이그레이션](../../compute/migration/)과, 대용량 데이터 물리 전송은 [스토리지 마이그레이션](../../storage/migration/)과 연결됩니다. 이 문서는 데이터베이스 이전(스키마·데이터·다운타임 최소화)에 초점을 둡니다.
:::
