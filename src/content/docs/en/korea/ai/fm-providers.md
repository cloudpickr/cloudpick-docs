...
| Upstage | Solar | Solar Pro 4 (agent-workload focused, 512K context, announced Aug 2026) is the current flagship. Predecessors Solar Pro 3 (Jan 2026), Solar Pro 2 (Jul 2025). Lightweight Solar Mini 4 (Sep 2026) and open-weight Solar Open 2 offered alongside | Pro line is closed API; Open line is open-weight | Upstage Console API, OpenRouter, Amazon Bedrock Marketplace, AWS SageMaker JumpStart, AWS Marketplace |
...
- **Cost**: Upstage's Solar Pro 4 has been published at around $0.30 input / $1.20 output per million tokens, and domestic models sometimes lead with price competitiveness. However, throughput and context length vary, so don't compare providers on token price alone.
