...
패턴을 아는 것보다 **언제 어떤 기법을 쓸지** 판단하는 것이 실무의 핵심입니다. 상황·모델 특성·비용을 함께 고려합니다.

| 상황 | 권장 기법 | 이유 / 주의 |
| --- | --- | --- |
| 단순 분류·추출·변환 | Zero-shot | 예시 없이도 충분. 토큰 낭비 최소화 |
| 출력 형식·톤이 까다로움 | Few-shot | 예시 2–5개로 형식을 "보여주기". 예시가 편향되지 않게 |
| 다단계 추론·수학·논리 | Chain-of-Thought | 중간 추론을 유도. **단, 추론 특화 모델(o-시리즈, 추론 모드)은 CoT를 내장하므로 명시적 CoT가 불필요하거나 역효과** |
| 구조화 출력(JSON 등)이 필수 | 출력 형식 강제 + 스키마 | 프롬프트 지시보다 **네이티브 구조화 출력/함수 호출 API**(지원 시)가 더 신뢰성 높음 |
| 외부 도구·검색과 결합 | ReAct / 도구 호출 | 에이전트 워크플로. 루프 비용·무한루프 방지 장치 필요 |
| 일관된 역할·정책 적용 | System Prompt / Persona | 세션 전반에 고정. 사용자 입력으로 덮어쓰이지 않게 |

**모델 유형별 고려:**

- **일반 모델(GPT-5.6, Claude Sonnet 등)**: Few-shot·CoT가 품질을 유의미하게 끌어올림.
- **추론 특화 모델(o-시리즈, Claude 추론 모드 등)**: 내부적으로 추론하므로 "단계별로 생각하라" 류 지시가 불필요하고, 과도한 CoT 지시는 오히려 성능·비용에 불리할 수 있음.
- **비용 관점**: Few-shot 예시와 CoT는 입력 토큰을 늘립니다. 반복 호출이 많은 경로에서는 예시 수·추론 길이를 최소화하거나 프롬프트 캐싱을 활용합니다.

:::note
어떤 기법이 실제로 나은지는 **평가로 확인**해야 합니다. 대표 입력 세트에 대해 정답률·형식 준수율·토큰 비용을 A/B로 측정하고, 아래 [개선 반복 루프](#프롬프트-개선-반복-루프)로 이어가세요.
:::
