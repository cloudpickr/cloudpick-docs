...
:::note[역할별로 어디부터 읽을까]
AI 도입은 역할마다 필요한 정보와 깊이가 다릅니다. 아래는 빠른 진입점이며, 역할×성숙도(도입 단계)를 교차한 상세 경로는 [AI 시스템 수명주기](../../ai/lifecycle/)의 도입 여정 지도를 참고하세요.

- **경영·의사결정자** — [AI 플랫폼과 모델 비교](../../ai/ai-ml/), [AI 시스템 수명주기](../../ai/lifecycle/) (무엇을·왜)
- **아키텍트** — [AI 시스템 수명주기](../../ai/lifecycle/), [멀티클라우드 AI](../../ai/multicloud-ai/)
- **개발자** — [프롬프트 엔지니어링](../../ai/prompt-engineering/), [RAG 고급 패턴](../../ai/rag-patterns/), [AI 에이전트](../../ai/agents/)
- **운영자·플랫폼** — [LLMOps](../../ai/llmops/), [AI 보안](../../security/ai-security/)
- **조달·FinOps** — [LLM 채널 선택](../../ai/1p-vs-3p/), [라이선스와 비용](../../ai/licensing/)
- **거버넌스** — [에이전트 도입 가이드](../../ai/agent-adoption/), 수명주기의 [문제 정의·거버넌스](../../ai/lifecycle/#1-문제-정의-및-거버넌스-problem-framing--governance)
- **현장 배포(FDE)** — [현장 배포](../../about-cloud/field-deployment/) (고객 환경 프로덕션 딜리버리)
:::
...
가장 간단한 시작점입니다. Amazon Bedrock, Microsoft Foundry, Gemini Enterprise, OCI Enterprise AI 중 하나의 API로 질문을 보내고 답을 받습니다. 챗봇·문서 요약·번역 같은 용도에 바로 쓸 수 있습니다. 벤더별 서비스 비교는 [AI 플랫폼과 모델 비교](../../ai/ai-ml/)를, 유용한 유스케이스 전반은 [이런 상황에서 유용합니다](../../ai/ai-ml/#이런-상황에서-유용합니다)를 참고하세요.

### 단계 2: 프롬프트 엔지니어링

같은 API라도 어떻게 묻느냐에 따라 답이 크게 달라집니다. 역할과 지시를 명확히 주면 코드 작성 없이도 품질이 올라갑니다. 설계 기법 상세는 [프롬프트 엔지니어링](../../ai/prompt-engineering/)을 참고하세요.

### 단계 3: RAG (Retrieval Augmented Generation)

파운데이션 모델은 일반 지식은 풍부하지만 **내 회사 데이터는 모릅니다**. RAG는 내 문서를 검색해 관련 부분을 프롬프트에 포함시킨 후 모델에게 전달하는 기술입니다(비유: "오픈북 시험"). 사내 문서 기반 챗봇, 제품 FAQ, 법률·의료 문서 조회 등에 쓰입니다.
