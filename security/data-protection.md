...
3사 모두 관리형 서비스 간 통신은 기본적으로 TLS로 암호화됩니다.
...
| 항목 | AWS | Azure | GCP |
| --- | --- | --- | --- |
| **기본 암호화** | 대부분 서비스 기본 활성화 (S3, EBS 등) | 모든 서비스 기본 활성화 | 모든 데이터 기본 암호화 |
| **키 관리** | KMS (벤더 관리 키 / 고객 관리 키) | Key Vault | Cloud KMS |
| **고객 키(CMK/CMEK)** | KMS Customer Managed Key | Key Vault Customer Key | Cloud KMS CMEK |
| **자체 키(BYOK)** | KMS External Key Store | Key Vault BYOK | Cloud External Key Manager (EKM) |
