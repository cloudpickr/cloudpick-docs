...
:::note
モデル選定については[AIプラットフォームとモデル比較](../../ai/ai-ml/)を、RAGパイプライン構築については[RAG高度なパターン](../../ai/rag-patterns/)を、AIセキュリティについては[AIセキュリティ](../../security/ai-security/)を参照してください。
:::
...
| 指標 | 測定対象 | 意味 |
| --- | --- | --- |
| **Retrieval Precision** | 検索された文書のうち関連文書の割合 | 不要な文書が混ざっていないか |
| **Retrieval Recall** | 関連文書のうち実際に検索された割合 | 必要な文書を見逃していないか |
| **Faithfulness** | 応答が検索された文書に基づいているか | ハルシネーション(Hallucination)の有無 |
| **Answer Relevance** | 応答が質問に適切か | 質問と無関係な回答の有無 |
...
- **プロンプト/応答ログにPIIをそのまま保存** — マスキングなしにログを保存し、個人データ保護規制(GDPR、個人情報保護法など)違反およびデータ漏洩のリスク
