...
:::note
벤더별 제품 비교, 인증 방식, 권한 모델 개요는 [IAM 개요](../../about-cloud/iam-overview/)를 참고하세요.
:::
...
최소 권한 원칙을 지키려면, 실제 사용되는 권한을 모니터링하고 불필요한 권한을 지속적으로 제거해야 합니다. IAM 이상 행위 탐지(비정상 API 호출 등)는 [보안 태세 관리](../../security/security-posture/)의 위협 탐지 서비스와 연동됩니다.
