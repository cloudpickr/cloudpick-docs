...
**망분리**(Network Segregation)는 업무망과 인터넷망을 분리하여 외부 위협이 내부 시스템에 도달하지 못하도록 하는 통제 방식입니다. 인터넷을 통한 악성코드 유입, 원격 침투, 데이터 유출을 네트워크 경계에서 원천 차단한다는 점에서, 여러 국가의 금융·공공·의료 등 규제 시장이 오랫동안 핵심 보안 요건으로 채택해 왔습니다. 특히 한국은 공공·금융 부문 보안 정책의 근간으로 망분리를 운용해온 대표적인 시장입니다 — 한국 규제(전자금융감독규정, CSAP, N2SF)의 상세와 최근 완화 동향은 [망분리와 네트워크 격리 (한국)](../korea/security/network-isolation.md)에서 다룹니다.

API 연동, SaaS 활용, 원격 근무 등 시스템 간 연결이 불가피해진 현대 환경에서는 "분리"의 의미와 구현 방식이 함께 진화하고 있습니다.
...
| **규제 적용** | 최고 민감도 등급 시스템 (예: 한국 CSAP 상 등급, 일부 금융 핵심 시스템) | 대부분의 클라우드 워크로드 (예: 한국 CSAP 중/하 등급) |
...
인터넷과 완전히 분리된 클라우드 환경이 필요한 경우, 각 벤더는 다음 옵션을 제공합니다. 단, 각국 규제 시장에서의 인증 취득 여부(예: 한국 CSAP)는 별도로 확인해야 합니다.
...
규제 시장의 망분리·격리 정책은 세계적으로 "모든 시스템을 동일 수준으로 분리"하는 일률적 접근에서, **데이터 등급에 따른 차등 보안**으로 정교화되고 있습니다. "분리 여부"의 이분법이 아니라 "어떤 수준의 통제가 필요한가"가 정책 질문이 되는 흐름입니다.

| 규제/기준 | 요구사항 | 클라우드 대응 |
| --- | --- | --- |
| **PCI DSS** (카드 결제, 글로벌) | CDE(카드 데이터 환경) 네트워크 격리 | 전용 VPC + 방화벽 + 로깅 |
| **국가별 공공·금융 규제** | 국가마다 물리적/논리적 분리 요건과 등급 체계가 다름 | 국가별 문서에서 상세 매핑 |

국가별 규제 상세는 해당 국가 문서를 참고하세요.

- **한국** — 전자금융감독규정, CSAP 상·중·하 등급, ISMS-P, N2SF(국가망보안체계) C/S/O 등급: [망분리와 네트워크 격리 (한국)](../korea/security/network-isolation.md)

{% hint style="info" %}
어느 규제 체계든 대부분의 기관은 모든 시스템이 같은 등급이 아닙니다. 시스템별로 등급을 분류하고, 등급에 맞는 격리 수준을 차등 적용하는 것이 핵심입니다. "전부 최고 등급"이나 "전부 퍼블릭"이 아니라, 혼합 구성이 현실적입니다.
{% endhint %}
...
- 한국 규제(전자금융감독규정, CSAP, N2SF) 원문 링크: [망분리와 네트워크 격리 (한국)](../korea/security/network-isolation.md)
