...
:::caution
국외 리전을 DR 대상으로 사용할 경우, 해당 국가의 개인정보 보호 법제가 정한 데이터 국외 이전 요건을 충족해야 합니다. 데이터 주권이 엄격한 워크로드는 국내 DR이 가능한 벤더·리전 조합을 우선 검토하세요. 국가별 리전 현황과 국내 DR 가능 조합은 국가별 가이드를 참고하세요 — [한국](../../korea/index/) 등.
:::
