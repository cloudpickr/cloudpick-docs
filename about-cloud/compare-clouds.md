...
네이버의 AI 기술(HyperCLOVA X)과 검색·커머스 인프라 운영 경험을 기반으로 합니다. 공공 부문 CSAP 상 등급 인증으로 민감 데이터 처리가 가능하며, 글로벌 리전도 확장 중입니다.

- **핵심 강점:** HyperCLOVA X(한국어 AI), CSAP 상 등급, 글로벌 리전 확장
- **차별점:** 네이버 서비스 연동(CLOVA, Papago, Maps), 한국어 기술 지원 우수
- **적합한 워크로드:** 공공기관, 한국어 AI 서비스, 네이버 생태계 연동
...
- **적합한 워크로드:** AI/ML 학습, Kakao 서비스 연동, 스타트업

## CSAP 인증과 벤더 선택

한국 공공기관에 클라우드를 도입하려면 CSAP(클라우드 보안 인증 제도) 인증 벤더를 사용해야 합니다.

| 등급 | 대상 시스템 | 인증 벤더 (2025년 기준) |
| --- | --- | --- |
| **상** | 민감 정보 처리 | NCP, KT Cloud, NHN Cloud |
| **중** | 일반 업무 시스템 | 위 + AWS, Azure, GCP, Kakao Cloud |
