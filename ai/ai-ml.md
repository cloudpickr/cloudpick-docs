...
AI는 모델 API 호출을 넘어 산업별 완성형 서비스(Applied AI), 물리 세계 연결(Physical AI), 자율 에이전트(Agentic Apps)로 확장되고 있습니다.

{% hint style="info" %}
Applied AI(컨택센터, 문서 처리, BI), Physical AI(IoT, 로보틱스, 디지털 트윈), Agentic Apps의 상세 비교는 [AI 활용 패턴](ai-use-cases.md)을 참고하세요.
{% endhint %}
