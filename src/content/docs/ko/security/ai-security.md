...
:::note[선행 지식 및 연결 문서]
AI 서비스·플랫폼 기본은 [AI 시작하기](../../ai/getting-started/), 멀티클라우드 AI 설계는 [멀티클라우드 AI](../../ai/multicloud-ai/)를 먼저 참고하세요. 이 문서는 프롬프트 인젝션·민감정보 유출·에이전트 권한 등 AI 고유의 보안 위협과 가드레일에 초점을 둡니다.
:::

AI 서비스를 프로덕션에 배포하면 전통적 보안 위협 외에 **AI 고유의 보안 위협**이 추가됩니다. [멀티클라우드 AI](../../ai/multicloud-ai/)에서 설계 시 주의사항으로 언급한 내용을 여기서 상세히 다룹니다.

## 주요 위협

아래 위협 분류는 실무 관점 요약이며, 적대적 머신러닝·GenAI 공격 클래스의 표준 분류는 [NIST AI 100-2 e2025](https://csrc.nist.gov/pubs/ai/100/2/e2025/final)를 참고하세요.
...
- [ ] [EU AI Act](../../governance/compliance/) GPAI·고위험 AI 의무 해당 여부와 준수 계획을 확인했는가
...
| **2026.08.02** | 제50조 투명성 의무 발효(합성 콘텐츠·딥페이크 표시) + GPAI 제재 권한 발동. **고위험 AI(Annex III) 의무는 Digital Omnibus로 2027.12.2로 연기** ([EU AI Act 전문](https://artificialintelligenceact.eu/), [상세](../../eu/nis2-ai-act/)) |

**클라우드 운영 시 영향:**

- GPAI 모델을 사용하는 경우, 벤더가 제공하는 **모델 카드(Model Card)** 와 투명성 문서를 확인해야 합니다
- 생성형 AI 출력물(이미지·오디오·비디오·텍스트)은 제50조에 따라 기계 판독 가능한 표시가 필요합니다(기존 출시 시스템은 2026.12.2까지 유예)
- 고위험으로 분류된 AI 시스템은 로깅, 인간 감독, 위험 평가가 법적 의무가 됩니다(적용 시점은 2027.12로 연기되었으나 준비는 조기 착수 권장)
...
- [AI 에이전트](../../ai/agents/) — 에이전트 아키텍처, 가드레일 구현, 보안 위협 요약
- [시크릿 관리](../../security/secrets/) — 에이전트가 사용하는 API 키/토큰 보호
- [IAM 실무 설계](../../security/iam/) — 에이전트 도구 호출 시 최소 권한 설정
