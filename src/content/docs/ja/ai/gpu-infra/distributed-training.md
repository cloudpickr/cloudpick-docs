...
:::note
ここに出てくる並列化方式とフレームワーク（DeepSpeed、Megatron-LM、PyTorch FSDP）はCUDA/NCCLの上で動き、ベンダーに縛られません。つまりクラウドを変えても戦略自体はおおむねそのまま移ります。ただし実際の速度は[ノード間通信網の性能](../../ai/gpu-infra/workload-and-architecture/#ノード間高速通信ファブリック--ベンダーマッピング)に左右されます。
:::
...
モデル全体をGPUごとに同じく複製しておき、データのまとまりだけ分けて各自処理した後、結果（勾配）を互いに突き合わせます。最も単純で、モデルがGPU 1枚に収まるときの標準として使います。

- **通信方式** — 学習ステップごとに全GPUが計算結果を合わせます。（all-reduce = すべてのGPUの値を集めて合計し、再び全員に配り直す通信。）
...
レイヤー（モデルを構成する計算の層）一つの計算を複数のGPUが同時に分けて処理します。一つの層がGPU 1枚に入らないほど大きいときに使います。

- **通信方式** — 層の内部でGPU同士が非常に頻繁にやり取りします。そのため遅延（反応の速さ）に非常に敏感です。
- **適用範囲** — 通信が頻繁なため、主に**1台のサーバー内**（NVLinkで繋がれたGPU群）でのみ使います。
- **効果** — 層一つがGPUメモリを超えるときに必須。

## パイプライン並列 (Pipeline Parallelism, PP)

モデルの層をいくつかの段階（stage）に分けて別々のGPUグループに配置し、データを小さな断片（マイクロバッチ）に刻んでリレーのように流します。

- **通信方式** — 段階と段階が出会う境界でのみ結果を渡します。そのため通信量が相対的に少ないです。
- **適用範囲** — 通信が少なく**複数のサーバーへ広げやすい**です。
