...
**網分離**(Network Segregation)は、業務網とインターネット網を分離することで外部の脅威が内部システムに到達しないようにする統制方式です。インターネット経由のマルウェア流入、リモート侵入、データ漏洩をネットワーク境界で根本的に遮断するという点で、多くの規制市場が長年セキュリティポリシーの基盤として採用してきました。

金融・公共・医療など規制産業において中核的なセキュリティ要件として適用されています(各国の根拠規定と等級体系は上部の国別ガイドを参照)。ただし、API連携、SaaS活用、リモートワークなどシステム間の接続が不可避となった現代の環境では、「分離」の意味と実装方式もともに進化しています。
...
| **規制適用** | 最上位の機密性等級 (機密・基幹システム) | 大半のクラウドワークロード (国別認証等級のマッピングは国別ガイドを参照) |
...
インターネットと完全に分離されたクラウド環境が必要な場合、各ベンダーは以下のオプションを提供しています。ただし、これらのオプションが各国の公共認証体系(例: 韓国CSAP、米国FedRAMP)で認められるかは別途確認する必要があります。
...
細部の要件は管轄区域ごとに異なりますが、方向性は共通しています。各国のネットワーク隔離政策は、「すべてのシステムを同一水準で分離」する画一的アプローチから、**データ等級に応じた差等セキュリティ**へと精緻化が進んでいます。「分離するかどうか」の二分法ではなく、「どの水準の統制が必要か」が政策上の問いになりつつあります。

| 規制類型 (例) | 要求事項 | クラウド対応 |
| --- | --- | --- |
| **カード決済** (PCI DSS — グローバル) | CDE(カードデータ環境)のネットワーク隔離 | 専用VPC＋ファイアウォール＋ロギング |
| **金融の網分離規制** (国別) | 内部網/DMZ/外部網の区分、電算室の分離 | VPC分離＋Private Subnet＋専用線 |
| **公共クラウド認証** (国別の等級制) | 等級に応じた物理的/論理的分離 | 等級別の隔離水準を差等適用 |
| **情報保護管理体系** (全産業) | ネットワーク分離、アクセス統制、暗号化 | Security Group + NACL + VPC Endpoint + TLS |

:::note
大半の組織では、すべてのシステムが同じ等級ではありません。システムごとに等級を分類し、等級に見合った隔離水準を差等適用することが核心です。「すべて最上位等級」や「すべてパブリック」ではなく、混合構成が現実的です。国別の等級体系とアーキテクチャマッピング(韓国CSAP・N²SF・電子金融監督規程、米国FedRAMP・ITAR、EU DORAなど)は上部の国別ガイド文書を参照してください。
:::
...
- 国別の規制・認証の出典は各国別ガイドの「参考資料」を確認してください — [韓国](../../korea/security/network-isolation/) · [米国](../../us/index/) · [EU](../../eu/index/)
