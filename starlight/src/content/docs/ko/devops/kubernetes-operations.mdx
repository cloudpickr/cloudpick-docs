...
[컨테이너 서비스](../../compute/containers/)에서 관리형 Kubernetes를 선택한 뒤에는 **Day-2 운영**이 시작됩니다. 클러스터 업그레이드, 배포 자동화, 보안 정책, 관측가능성 등 지속적으로 관리해야 할 영역입니다.

:::note
관리형 Kubernetes 선택 가이드는 [컨테이너 서비스](../../compute/containers/)를, 서비스 간 통신 보안은 [서비스 메시](../../compute/service-mesh/)를 참고하세요.
:::
...
Git 저장소를 단일 진실 원천으로 사용하여 클러스터 상태를 선언적으로 관리합니다. GitOps 개념과 도구 비교는 [DevOps 시작하기](../../devops/getting-started/#gitops)를, CI/CD 파이프라인 설계는 [CI/CD](../../devops/cicd/)를 참고하세요.
...
:::note
VPC/서브넷 설계 기초(CIDR 계획, 퍼블릭/프라이빗 분리, Transit Gateway)는 [VPC와 서브넷](../../networking/vpc-subnet/)을, 관리형 Kubernetes 선택과 노드 전략은 [컨테이너 서비스](../../compute/containers/)를 참고하세요.
:::
