...
[보안 태세 관리](../../security/security-posture/)로 예방하고 탐지하더라도, 보안 사고는 발생할 수 있습니다. 클라우드 환경에서는 온프레미스와 다른 대응 절차가 필요합니다 — 계정 격리, 토큰 회수, API 기반 증거 보존, 자동 격리 등.

## 대응 흐름

:::caution
보안 사고 대응은 **사고가 발생한 뒤에 준비할 수 없습니다.** 런북(대응 절차서), 역할 분담, 연락 체계, 격리 스크립트를 사전에 작성하고 정기적으로 훈련해야 합니다. 준비 없이 사고를 맞으면 판단 지연, 증거 유실, 피해 확대로 이어집니다.
:::
...
- **사고 발생 후에야 런북을 작성** — 사전 준비 없이 사고를 맞으면 판단 지연, 증거 유실, 피해 확대로 이어짐
- **침해된 인스턴스를 즉시 종료(terminate)** — 메모리, 디스크, 네트워크 연결 정보가 사라져 포렌식이 불가능해짐. 격리 후 스냅샷 먼저 확보해야 함
- **감사 로그 보존 기간이 짧음** — 기본 90일 보존으로 두어 사고 조사 시 과거 로그가 이미 삭제된 상태

## 체크리스트

- [ ] 사고 유형별 런북(IAM 키 유출, 데이터 유출, 랜섬웨어)을 사전에 작성하고 정기 훈련을 수행하는가
- [ ] 포렌식 전용 계정을 분리하고, 감사 로그를 최소 1년 이상 변조 방지 설정으로 보존하는가
- [ ] Break-glass(비상 접근) 계정을 준비하고, 사용 시 사후 감사 절차가 정의되어 있는가
