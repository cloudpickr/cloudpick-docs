...
:::note[前提知識と関連ドキュメント]
ファウンデーションモデルとプラットフォームの基本は[AIを始める](../../ai/getting-started/)、同一のFMをどこで消費するか(1P直接 vs 3Pクラウド経由)は[LLMチャネル選択ガイド](../../ai/1p-vs-3p/)を先に参照してください。本ドキュメントは、FMのライセンス等級とコスト管理に焦点を当てます。
:::
...
| --- | --- | --- | --- |
| **Business** (旧Team) — Standard席 | ~$25/席/月 (年払い $20) | 小規模チーム (2席+) | — |
| **Business** — Premium席 | ~$125/席/月 (年払い $100) | 高使用量ユーザー | Standard比5倍の使用量、5時間の使用制限なし。1つのワークスペースで席タイプを混在可能 |
...
:::note
上記の表は組織レベルの**課金モデル・調達ガバナンス**(誰がいくら使い、どう配分・請求するか)に焦点を当てます。プロダクションランタイムでリクエスト単位に予算を強制し(Budget Guardrail)、モデル障害時に切り替える(Fallback)運用制御は[LLMOps](../../ai/llmops/)を、モデル別の推論単価削減(プロンプトキャッシュ・バッチ・モデルルーティング)は[AIプラットフォームとモデル比較 — 推論コスト最適化](../../ai/ai-ml/#推論コスト最適化)を参照してください。
:::
...
- [LLMOps](../../ai/llmops/) — ランタイムコスト運用(Budget Guardrail、Fallback、リクエストあたりコスト)
