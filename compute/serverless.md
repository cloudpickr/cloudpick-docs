...
flowchart TD
    ...
    B -->|아니오| F{실행 시간?}
    F -->|15분 이하| G{트래픽 패턴?}
    G -->|간헐적/이벤트 기반| H[서버리스 함수]
    G -->|상시 트래픽| I[서버리스 컨테이너]
    F -->|15분 초과| J[컨테이너]
```

| 이럴 때 | 이것을 선택 |
| --- | --- |
| 이벤트 기반, 짧은 실행, 유휴 시 비용 0 | 서버리스 함수 (Lambda, Functions, Cloud Functions) |
| 상시 트래픽 + 서버 관리 불필요 | 서버리스 컨테이너 (Fargate, Cloud Run, Container Apps) |
| 복잡한 오케스트레이션, 상태 유지, 세밀한 제어 | 관리형 Kubernetes (EKS, AKS, GKE, OKE) |
| OS/커널 접근, 레거시 앱, 특수 하드웨어 | [가상머신](virtual-machines.md) |
