...
:::note
キャッシュ運用時に注意すべきアンチパターン(永続ストレージのように使用、TTL未設定、キャッシュ依存アーキテクチャなど)は、[データベース運用 — キャッシュアンチパターン](../../database/operations/#캐시-운영-주의사항)を参照してください。
:::
...
| 要件 | 推奨 |
| --- | --- |
| DB読み取り負荷分散(一般的なキャッシュ) | ElastiCache/Memorystore Redis(Cache-Aside) |
| セッションストア(TTL + 構造化データ) | Redis(Hashタイプ) |
| プライマリDBの代替(耐久性が必要) | MemoryDB |
| シンプルなkey-value、最大スループット | Memcached |
| リアルタイムリーダーボード/カウンター | Redis(Sorted Set) |
