...
:::note[선행 지식 및 연결 문서]
트랜잭션 데이터 저장은 [관리형 RDB](../../database/managed-rdb/), [NoSQL](../../database/nosql/)을 먼저 참고하세요. 이 문서는 전문 검색·벡터 검색·로그 분석 등 검색 엔진 워크로드에 초점을 둡니다. 생성형 AI 임베딩 파이프라인 관점의 벡터 스토어는 [벡터 스토어](../../ai/vector-store/)와 연결됩니다.
:::

### 현업에서 검색이 필요한 곳

| 사용 사례 | 설명 | 왜 DB의 LIKE/WHERE로 안 되는가 |
| --- | --- | --- |
...
벡터 검색 상세는 [벡터 스토어](../../ai/vector-store/)를 참고하세요.
...
| Azure | [Azure AI Search](https://learn.microsoft.com/azure/search/) | AI Search (벡터 + 키워드 + 시맨틱) | [Log Analytics](https://learn.microsoft.com/azure/azure-monitor/logs/data-platform-logs) |
...
| AI 기반 질의응답 (RAG) | [벡터 스토어](../../ai/vector-store/) + 하이브리드 검색 |
...
| 다국어 및 CJK 형태소 분석 필요 | OpenSearch (nori, kuromoji 플러그인), Azure AI Search (언어별 분석기) |

## 흔한 실수

- **RDB의 LIKE 검색으로 전문 검색을 대체하려는 것** — 데이터가 증가하면 풀 테이블 스캔으로 성능이 급격히 저하됩니다. 전문 검색이 필요하면 검색 엔진을 도입하세요.
- **샤드를 과도하게 설정하는 것** — OpenSearch/Elasticsearch에서 샤드가 너무 많으면 클러스터 오버헤드가 증가합니다. 샤드당 10–50GB 기준으로 설계하세요.
- **로그 보존 기간을 무제한으로 두는 것** — 검색 엔진에 모든 로그를 영구 보관하면 스토리지 비용이 폭증합니다. 핫/웜/콜드 계층을 활용하고 오래된 로그는 객체 스토리지로 이전하세요.

## 점검 목록

- [ ] 검색 요구사항(키워드/시맨틱/하이브리드)을 정의하고 적합한 서비스를 선택했는가
- [ ] 인덱스 라이프사이클 정책(ILM)과 로그 보존 기간을 설정했는가
- [ ] 다국어(CJK 등) 검색이 필요한 경우 적합한 형태소 분석기(nori, kuromoji 등)를 구성했는가
...
- [Log Analytics 문서](https://learn.microsoft.com/azure/azure-monitor/logs/data-platform-logs)
