...
:::tip[この文書はAI導入のジャーニーマップです]
「なぜAIか → 何を買うか → どう作るか → どう運用・ガバナンスするか → 現場へデプロイ」の全サイクルを本文書が俯瞰します。[AIを始める](../../ai/getting-started/)で役割別のクイック入口を選んだうえで、本文書の[4段階導入マトリクス](#4段階エンタープライズ導入マトリクス)で自分の役割・成熟度に合った道筋を確認してください。
:::
...
- **学習パイプライン構築** — 独自モデルをトレーニングまたはFine-tuningする場合、分散学習パイプラインを構築します。並列化戦略（TP/PP/DP）とGPUクラスター構成は[GPUインフラ — 分散学習の標準アーキテクチャ](../../ai/gpu-infra/distributed-training/)を参照してください。
...
:::note[現場デプロイ (FDE) との連携]
設計されたAIアーキテクチャを顧客の閉域網・エアギャップ環境や複雑なレガシー基盤に本番コードとして定着させる実行モデルの一つとして、[現場デプロイ (Field Deployment)](../../about-cloud/field-deployment/)を参照できます。
:::
...
自分のTier・役割を決めたら、次の実務文書へ進みます。

- **経営・意思決定者（Tier 1〜2の判断）** — [AIプラットフォームとモデル比較](../../ai/ai-ml/)で候補を絞り、[LLMチャネル選択](../../ai/1p-vs-3p/)・[ライセンスとコスト](../../ai/licensing/)で調達方式を決めます。
- **アーキテクト（Tier 2〜3の設計）** — ここでモデル・制御境界を定めたうえで、配置アーキテクチャは[マルチクラウドAI](../../ai/multicloud-ai/)へ、セキュリティ境界は[AIセキュリティ](../../security/ai-security/)へ進みます。
- **開発者（Tier 3の構築）** — [プロンプトエンジニアリング](../../ai/prompt-engineering/) → [RAG高度パターン](../../ai/rag-patterns/) → [AIエージェント](../../ai/agents/)の順に実装します。
- **運用・プラットフォーム（Tier 3〜4の運用）** — [LLMOps](../../ai/llmops/)で評価・コスト・観測の体系を整えます。
- **ガバナンス** — 本文書の[問題定義・ガバナンス](#1-問題定義およびガバナンス-problem-framing--governance)段階と、[エージェント導入ガイド](../../ai/agent-adoption/)のロールアウト・ガバナンスフレームワークを併せて参照します。
- **現場デプロイ（FDE）** — 設計されたシステムを顧客の閉域網・レガシー環境に本番コードとして定着させる観点は[現場デプロイ](../../about-cloud/field-deployment/)を参照します。
...
- **インフラのコード化 (IaC)** — ベクトルデータベース、GPUノードプール、サービングエンドポイントを[IaC](../../devops/iac/)でプロビジョニングし、環境の一貫性を保証します。GPUノードプールのスケジューリング・クォータ・共有は[GPUインフラ — GPU Kubernetesとスケジューリング](../../ai/gpu-infra/kubernetes-and-scheduling/)を参照してください。
