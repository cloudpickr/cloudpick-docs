...
AIの導入方法は、解決したいビジネス課題だけでなく、**利用主体（非エンジニア vs 開発者 vs MLエンジニア）**と**UI/UXの接点形態**によって大きく異なります。

| 段階（導入形態） | 対象ユーザー（ペルソナ） | UI/UX接点形態 | 代表的なサービス例 | 適したユースケース |
| --- | --- | --- | --- | --- |
| **1. Buy（完成型SaaS）** | 非エンジニア・一般社員（ノーコード） | 業務ソフト内蔵型Copilot、全社WebチャットUI | Microsoft 365 Copilot、Gemini for Workspace、Salesforce Agentforce | メール・報告書のドラフト作成、議事録要約、CRM顧客対応 |
| **2. Assemble（ノーコードStudio）** | 業務企画者、シチズンデベロッパー | ドラッグ＆ドロップのビジュアルキャンバス、Slack/Teamsボット連携 | Microsoft Copilot Studio、Bedrock IDE (SageMaker Unified Studio)、Vertex AI Agent Builder | 社内就業規則Q&Aボット、部門別定型データ収集ボット |
| **3. Build（カスタムアプリ・API）** | ソフトウェアエンジニア（開発チーム） | 独自Web/モバイルUI、バックグラウンド自律エージェント、バックエンドAPI | Amazon Bedrock API、Azure AI Foundry SDK、Gemini Enterprise API | 社内基幹システム連携エージェント、自社サービス組み込みAI機能 |
| **4. Train & Ops（フルスタックML）** | MLエンジニア、データサイエンティスト | Jupyter/VS Code、MLOpsオーケストレーションパイプライン | SageMaker AI、Azure Machine Learning、Vertex AI Pipelines | 不正検知（FDS）、大規模時系列需要予測、独自モデルのFine-tuning |

:::note
- **導入形態の本質**: 上記の4段階は順番に登る成熟度ラダーではなく、**運用負担(TCO)と必要な制御権(IP保護・カスタマイズ)のトレードオフ軸**です。
- **非エンジニア部門・一般業務の自動化**: 第1段階（Buy）または第2段階（Assemble）から始めるのが、導入コストと市場投入速度（Time to Market）の観点から最も効果的です。
- **社内システム連携や自社サービスの開発**: 第3段階（Build）のファウンデーションモデルAPIやエージェントSDKを活用します。
...
ここから先は、最も多く選ばれる**生成AI(FM API)** の導入順序を扱います。従来型MLパイプライン(学習→デプロイ→モニタリング)については[AIプラットフォームとモデル比較 — MLパイプライン](../../ai/ai-ml/#mlパイプラインとmlops)を参照してください。
