...
| **DSR (Direct Server Return)** | 응답 트래픽이 LB를 우회하여 클라이언트로 직접 전달 | 대부분 미지원 (아래 참고) |

{% hint style="info" %}
**DSR이 클라우드에서 쓰이지 않는 이유:** 온프레미스에서 DSR은 LB의 대역폭 병목을 피하기 위해 사용했습니다. 클라우드 LB는 자동 확장되어 대역폭 병목이 없으므로 DSR이 불필요합니다. 또한 클라우드 LB는 TLS 종료, 헬스 체크, 로깅을 응답 경로에서도 처리하므로, 응답이 LB를 경유해야 이 기능들이 동작합니다.
{% endhint %}
