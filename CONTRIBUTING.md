...
워커 소견과 최종 리포트는 아래 골격을 따릅니다. (실무에서 잘 동작한 형태를 기준으로 함)

```markdown
# 문서 리뷰 (YYYY-MM-DD)

> 오케스트레이터: … | 워커: … | 대상 브랜치/커밋: …

## 1. 오케스트레이션 로그
| 워커 | 범위 | 결과 | 소요 |

## 2. 기계 검사 결과
| 검사 | 결과 | 비고 |   # markdownlint, 내부 링크 등

## 3. Must-Fix (머지 차단)
각 항목: 파일:대략줄 | 문제 | 근거 | 제안
코디네이터 재검증 표시 (원문 grep/read 또는 공식 문서 확인)
...
| 3. Should-Fix | 처리 방침 | |
### 결론
한 줄 판정 (예: 머지 가능 / 머지 불가 — Must-Fix N건)

## 7. 이전 리뷰 대비 (해당 시)
| 새로 잡힌 이슈 | 해소 확인된 이슈 | 프로세스 관찰 |
```

- **Must-Fix** — 머지 전 해소 (구조 위반, 명백한 사실 오류, 문서 간 모순 수치, 깨진 내부 링크 등)
- **Should-Fix** — 같은 PR 또는 바로 다음 정리에서 처리 권장
- **Nice-to-Have** — 백로그

리뷰는 **파일 수정 없이** 소견만 내는 것을 기본으로 합니다. 수정은 작성자 또는 별도 작업에서 반영합니다.  
로컬 작업 메모는 `.reviews/`에 둘 수 있습니다 (gitignore, 본문 문서·SUMMARY에 넣지 않음).

### 멀티 에이전트 운용 (권장 스타일)

**한 명의 오케스트레이터**가 기준·분담·대기·합본·판정을 소유합니다. 워커는 맡은 범위만 리뷰하고, 최종 Must-Fix 목록은 코디네이터가 **원문을 다시 열어 재검증**한 뒤 확정합니다.

**분담 방식은 고정하지 않습니다.** 변경 범위·리스크·일정에 맞춰 그때그때 나눕니다.

| 상황 예 | 분담 예 (예시일 뿐) |
| --- | --- |
| 월간 전 문서 리뷰 | 도메인·섹션을 2~N개로 나눠 병렬 (예: 보안/거버넌스, AI/컴퓨트, 나머지+교차) |
| AI·모델 카탈로그만 갱신 | AI 전용 1~2 워커 + (필요 시) README/SUMMARY 교차 1 워커 |
| 보안/규제 패치 | `security/*`·`governance/*` 중심, 관련 교차 링크만 추가 |
| 이전 Must-Fix 해소 확인 | 워커 없이 코디가 해당 항목만 재검증 |
| 소규모 수정 | 멀티 에이전트 생략, 작성자 + 기계 검사 |

워커 수·경계·이름은 코디네이터가 정합니다. “항상 3분담”이 규칙은 아닙니다. 변동이 빠른 영역(모델명, GA/Preview, 가격)은 그 범위에 **전담 워커를 두는 편이 유리할 수 있다** 정도만 참고합니다.

운영 원칙:

1. 프롬프트에 이 절의 기준, “리뷰 전용 / 파일 수정 금지”, 위 산출 골격, **이번 분담표**를 넣습니다.
2. 독립 가능한 태스크는 **병렬로 먼저 띄운 뒤** 완료를 모읍니다. (순차 대기는 의존이 있을 때만)
3. 코디네이터는 워커 소견을 그대로 합치지 않고, **각 Must-Fix를 원문·공식 1차 소스로 재확인**합니다.
4. 최종 산출에 **오케스트레이션 로그(실제 분담 포함) + 기계 검사 + 머지 판정 표 + (가능하면) 이전 리뷰 대비**를 넣습니다.
5. 에이전트 간 이견·확인 불가는 공식 문서로 확인하거나, “GA 단정 금지 / 공식 링크 위임 / 표현 완화”로 처리합니다.
6. 이기종 CLI를 꼭 섞을 필요는 없습니다. 안정적으로 병렬 분담·재검증·단일 리포트가 나오면 됩니다. 연합 에이전트는 선택입니다.

위치 구분 (요약):

| 무엇 | 어디 |
| --- | --- |
| 내용 리뷰·오케스트레이션 | 로컬 에이전트 도구 (워크트리 위) |
| 기계 검사 | 로컬 동일 명령 + GitHub Actions `Docs Quality` |
| 리뷰 기준 | 이 문서 (CONTRIBUTING) — 코드 게이트로 소견을 고정하지 않음 |
...
| 2. 내용 리뷰 | 사람 또는 카운슬 코디네이터 | 이번 범위 **Must-Fix 0** (재검증 완료) |
...
국가 가이드를 나눈 이유는 **언어마다 국가 메시지가 달라지지 않게** 하기 위해서입니다.

- 글로벌 페이지는 모든 locale에서 **같은 포인터 집합**만 둡니다. "한국은 특히…"처럼 한 국가를 본문에서 강조하는 문장은 그 국가 가이드에만 둡니다.
- 국가 사실(리전 표, 인증 현황, 법령명, PoP)의 원본은 **루트 한국어 SOT**입니다. en/ja는 표 행·법령·포인터를 빼거나 보태지 않습니다. 검수는 한국어 원본을 기준으로 합니다.
