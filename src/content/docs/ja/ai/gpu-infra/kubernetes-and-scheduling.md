...
共有クラスターの悩みの種は、**あるチームがGPUを掴んで離さない（独占、hoarding）**状況です。すると他のジョブがGPUを受け取れず、飢え続けます。これを防ぐ仕組み:
...
分散学習は必要なGPUを**一度に全部**確保しないと始められません。たとえばGPU 16枚が必要なのに10枚だけ掴んで残り6枚を待つと、掴んだ10枚は何もできず資源だけを縛ります（ひどいと互いに待ち合って止まるデッドロック）。Gang schedulingはこれを防ぐため、**「必要なだけ全部揃えば開始、そうでなければまったく開始しない（全か無か）」**方式でスケジューリングします。「gang（一つの群れ）」を丸ごと掴むという意味です。
