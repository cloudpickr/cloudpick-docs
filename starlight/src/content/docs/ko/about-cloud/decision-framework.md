...
- 주 사용자층이 있는 국가의 리전을 보유한 벤더를 우선
- 글로벌 서비스는 엣지 네트워크와 리전 커버리지 검토
- DR 시 인접 리전까지 고려 ([리전과 가용영역](../../about-cloud/regions-and-zones/) 참고)

### 3. 규제 및 규정 준수

- 공공·금융 부문: 각국의 클라우드 인증·감독 규정 (예: 한국 CSAP·전자금융감독규정, 미국 FedRAMP)
- 개인정보: 대상 시장의 보호 법제 (예: EU GDPR)
- 자세한 내용은 [규정 준수](../../governance/compliance/)와 국가별 가이드 참고

### 4. 팀 역량과 생태계

- 기존 Microsoft 스택(AD, Office 365)이 있으면 Azure가 자연스럽습니다.
- 팀이 AWS에 익숙하면 초기 생산성이 높습니다.
- 주요 서비스의 현지 언어 공식 문서와 커뮤니티 지원도 고려합니다.
...
- 리전별 가격 차이 — 아시아 등 일부 리전은 미국 리전 대비 10~30% 높은 것이 일반적
- 관리형 서비스 프리미엄
